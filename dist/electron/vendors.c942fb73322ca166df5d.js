(this["webpackJsonp"] = this["webpackJsonp"] || []).push([
  [210],
  {
    2774: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2754)),
        s = _(n(3658)),
        a = _(n(2826)),
        u = o(n(2934)),
        i = o(n(3421)),
        c = o(n(3056)),
        l = o(n(4292)),
        d = _(n(2950)),
        f = n(3215),
        h = o(n(6093));
      function _(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (_ = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountId {
        constructor(t, e, n, o, r) {
          const a = s.constructor(t, e, n);
          ((this.shard = a.shard),
            (this.realm = a.realm),
            (this.num = a.num),
            (this.aliasKey = null != o ? o : null),
            (this.evmAddress = null != r ? r : null),
            (this._checksum = null));
        }
        static fromString(t) {
          let e = r.default.ZERO,
            n = r.default.ZERO,
            o = r.default.ZERO,
            a = void 0,
            u = void 0;
          if ((t.startsWith("0x") && 42 == t.length) || 40 == t.length)
            u = l.default.fromString(t);
          else {
            const c = s.fromStringSplitter(t);
            if (Number.isNaN(c.shard) || Number.isNaN(c.realm))
              throw new Error("invalid format for entity ID");
            (null != c.shard && (e = r.default.fromString(c.shard)),
              null != c.realm && (n = r.default.fromString(c.realm)),
              c.numOrHex.length < 20
                ? (o = r.default.fromString(c.numOrHex))
                : 40 == c.numOrHex.length
                  ? (u = l.default.fromString(c.numOrHex))
                  : (a = i.default.fromString(c.numOrHex)));
          }
          return new AccountId(e, n, o, a, u);
        }
        static fromEvmAddress(t, e, n) {
          const o = "string" === typeof n ? l.default.fromString(n) : n;
          return (0, f.isLongZeroAddress)(o.toBytes())
            ? this.fromSolidityAddress(o.toString())
            : new AccountId(t, e, 0, void 0, o);
        }
        static fromEvmPublicAddress(t) {
          return new AccountId(0, 0, 0, void 0, t);
        }
        static _fromProtobuf(t) {
          let e = void 0,
            n = void 0;
          return (
            null != t.alias &&
              (20 === t.alias.length
                ? (n = l.default.fromBytes(t.alias))
                : (e = u.default._fromProtobufKey(
                    a.proto.Key.decode(t.alias),
                  ))),
            e instanceof i.default || (e = void 0),
            new AccountId(
              null != t.shardNum ? t.shardNum : 0,
              null != t.realmNum ? t.realmNum : 0,
              null != t.accountNum ? t.accountNum : 0,
              e,
              n,
            )
          );
        }
        get checksum() {
          return this._checksum;
        }
        getEvmAddress() {
          return this.evmAddress;
        }
        async populateAccountNum(t) {
          if (null === this.evmAddress)
            throw new Error("field `evmAddress` should not be null");
          const e = t.mirrorNetwork[0].slice(
            0,
            t.mirrorNetwork[0].indexOf(":"),
          );
          await new Promise((t) => {
            setTimeout(t, 3e3);
          });
          const n = `https://${e}/api/v1/accounts/${this.evmAddress.toString()}`,
            o = (await h.default.get(n)).data.account;
          return (
            (this.num = r.default.fromString(o.slice(o.lastIndexOf(".") + 1))),
            this
          );
        }
        async populateAccountEvmAddress(t) {
          if (null === this.num)
            throw new Error("field `num` should not be null");
          const e = t.mirrorNetwork[0].slice(
            0,
            t.mirrorNetwork[0].indexOf(":"),
          );
          await new Promise((t) => {
            setTimeout(t, 3e3);
          });
          const n = `https://${e}/api/v1/accounts/${this.num.toString()}`,
            o = (await h.default.get(n)).data.evm_address;
          return ((this.evmAddress = l.default.fromString(o)), this);
        }
        validate(t) {
          (console.warn("Deprecated: Use `validateChecksum` instead"),
            this.validateChecksum(t));
        }
        validateChecksum(t) {
          if (null != this.aliasKey)
            throw new Error(
              "cannot calculate checksum with an account ID that has a aliasKey",
            );
          s.validateChecksum(
            this.shard,
            this.realm,
            this.num,
            this._checksum,
            t,
          );
        }
        static fromBytes(t) {
          return AccountId._fromProtobuf(a.proto.AccountID.decode(t));
        }
        static fromSolidityAddress(t) {
          return (0, f.isLongZeroAddress)(d.decode(t))
            ? new AccountId(...s.fromSolidityAddress(t))
            : this.fromEvmAddress(0, 0, t);
        }
        toSolidityAddress() {
          return null != this.evmAddress
            ? this.evmAddress.toString()
            : null != this.aliasKey && "secp256k1" == this.aliasKey._key._type
              ? this.aliasKey.toEvmAddress()
              : s.toSolidityAddress([this.shard, this.realm, this.num]);
        }
        _toProtobuf() {
          let t = null;
          return (
            null != this.aliasKey
              ? (t = a.proto.Key.encode(
                  this.aliasKey._toProtobufKey(),
                ).finish())
              : null != this.evmAddress && (t = this.evmAddress._bytes),
            {
              alias: t,
              accountNum: null != this.aliasKey ? null : this.num,
              shardNum: this.shard,
              realmNum: this.realm,
            }
          );
        }
        toBytes() {
          return a.proto.AccountID.encode(this._toProtobuf()).finish();
        }
        toString() {
          let t = this.num.toString();
          return (
            null != this.aliasKey
              ? (t = this.aliasKey.toString())
              : null != this.evmAddress && (t = this.evmAddress.toString()),
            `${this.shard.toString()}.${this.realm.toString()}.${t}`
          );
        }
        toStringWithChecksum(t) {
          if (null != this.aliasKey)
            throw new Error(
              "cannot calculate checksum with an account ID that has a aliasKey",
            );
          return s.toStringWithChecksum(this.toString(), t);
        }
        equals(t) {
          let e = !1;
          return (
            null != this.aliasKey && null != t.aliasKey
              ? (e = this.aliasKey.equals(t.aliasKey))
              : null != this.evmAddress && null != t.evmAddress
                ? (e = this.evmAddress.equals(t.evmAddress))
                : null == this.aliasKey &&
                  null == t.aliasKey &&
                  null == this.evmAddress &&
                  null == t.evmAddress &&
                  (e = this.num.eq(t.num)),
            this.shard.eq(t.shard) && this.realm.eq(t.realm) && e
          );
        }
        clone() {
          const t = new AccountId(this);
          return (
            (t._checksum = this._checksum),
            (t.aliasKey = this.aliasKey),
            (t.evmAddress = this.evmAddress),
            t
          );
        }
        compare(t) {
          let e = this.shard.compare(t.shard);
          if (0 != e) return e;
          if (((e = this.realm.compare(t.realm)), 0 != e)) return e;
          if (null != this.aliasKey && null != t.aliasKey) {
            const e = this.aliasKey.toString(),
              n = t.aliasKey.toString();
            return e > n ? 1 : e < n ? -1 : 0;
          }
          if (null != this.evmAddress && null != t.evmAddress) {
            const e = this.evmAddress.toString(),
              n = t.evmAddress.toString();
            return e > n ? 1 : e < n ? -1 : 0;
          }
          return null == this.aliasKey &&
            null == t.aliasKey &&
            null == this.evmAddress &&
            null == t.evmAddress
            ? this.num.compare(t.num)
            : 1;
        }
      }
      ((e.default = AccountId),
        c.default.setAccountIdConstructor(
          (t, e, n) => new AccountId(t, e, r.default.ZERO, n),
        ));
    },
    4678: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(2774)),
        a = o(n(2754));
      class TokenNftAllowance {
        constructor(t) {
          ((this.tokenId = t.tokenId),
            (this.spenderAccountId = t.spenderAccountId),
            (this.ownerAccountId = t.ownerAccountId),
            (this.serialNumbers = t.serialNumbers),
            (this.allSerials = t.allSerials),
            (this.delegatingSpender = t.delegatingSpender),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          const e = null != t.approvedForAll && 1 == t.approvedForAll.value;
          return new TokenNftAllowance({
            tokenId: r.default._fromProtobuf(t.tokenId),
            spenderAccountId:
              null != t.spender ? s.default._fromProtobuf(t.spender) : null,
            ownerAccountId:
              null != t.owner ? s.default._fromProtobuf(t.owner) : null,
            serialNumbers: e
              ? null
              : null != t.serialNumbers
                ? t.serialNumbers.map((t) => a.default.fromValue(t))
                : [],
            allSerials: e,
            delegatingSpender:
              null != t.delegatingSpender
                ? s.default._fromProtobuf(t.delegatingSpender)
                : null,
          });
        }
        static _fromGrantedProtobuf(t, e) {
          return new TokenNftAllowance({
            tokenId: r.default._fromProtobuf(t.tokenId),
            spenderAccountId: s.default._fromProtobuf(t.spender),
            ownerAccountId: e,
            serialNumbers: [],
            allSerials: null,
            delegatingSpender: null,
          });
        }
        static _fromRemoveProtobuf(t) {
          return new TokenNftAllowance({
            tokenId: r.default._fromProtobuf(t.tokenId),
            spenderAccountId: null,
            ownerAccountId:
              null != t.owner ? s.default._fromProtobuf(t.owner) : null,
            serialNumbers:
              null != t.serialNumbers
                ? t.serialNumbers.map((t) => a.default.fromValue(t))
                : [],
            allSerials: null,
            delegatingSpender: null,
          });
        }
        _toProtobuf() {
          return {
            tokenId: this.tokenId._toProtobuf(),
            spender:
              null != this.spenderAccountId
                ? this.spenderAccountId._toProtobuf()
                : null,
            owner:
              null != this.ownerAccountId
                ? this.ownerAccountId._toProtobuf()
                : null,
            approvedForAll:
              null == this.serialNumbers ? { value: this.allSerials } : null,
            serialNumbers: this.serialNumbers,
            delegatingSpender:
              null != this.delegatingSpender
                ? this.delegatingSpender._toProtobuf()
                : null,
          };
        }
        _validateChecksums(t) {
          (this.tokenId.validateChecksum(t),
            null != this.ownerAccountId &&
              this.ownerAccountId.validateChecksum(t),
            null != this.spenderAccountId &&
              this.spenderAccountId.validateChecksum(t));
        }
      }
      e.default = TokenNftAllowance;
    },
    5016: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(2831));
      class HbarAllowance {
        constructor(t) {
          ((this.spenderAccountId = t.spenderAccountId),
            (this.ownerAccountId = t.ownerAccountId),
            (this.amount = t.amount),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          return new HbarAllowance({
            spenderAccountId: r.default._fromProtobuf(t.spender),
            ownerAccountId:
              null != t.owner ? r.default._fromProtobuf(t.owner) : null,
            amount: s.default.fromTinybars(null != t.amount ? t.amount : 0),
          });
        }
        static _fromGrantedProtobuf(t, e) {
          return new HbarAllowance({
            spenderAccountId: r.default._fromProtobuf(t.spender),
            ownerAccountId: e,
            amount: s.default.fromTinybars(null != t.amount ? t.amount : 0),
          });
        }
        _toProtobuf() {
          return {
            owner:
              null != this.ownerAccountId
                ? this.ownerAccountId._toProtobuf()
                : null,
            spender:
              null != this.spenderAccountId
                ? this.spenderAccountId._toProtobuf()
                : null,
            amount: null != this.amount ? this.amount.toTinybars() : null,
          };
        }
        _validateChecksums(t) {
          (null != this.spenderAccountId &&
            this.spenderAccountId.validateChecksum(t),
            null != this.spenderAccountId &&
              this.spenderAccountId.validateChecksum(t));
        }
        toJSON() {
          return {
            ownerAccountId:
              null != this.ownerAccountId
                ? this.ownerAccountId.toString()
                : null,
            spenderAccountId:
              null != this.spenderAccountId
                ? this.spenderAccountId.toString()
                : null,
            amount: null != this.amount ? this.amount.toString() : null,
          };
        }
      }
      e.default = HbarAllowance;
    },
    5017: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(2774)),
        a = o(n(2754));
      class TokenAllowance {
        constructor(t) {
          ((this.tokenId = t.tokenId),
            (this.spenderAccountId = t.spenderAccountId),
            (this.ownerAccountId = t.ownerAccountId),
            (this.amount = t.amount),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          return new TokenAllowance({
            tokenId: r.default._fromProtobuf(t.tokenId),
            spenderAccountId: s.default._fromProtobuf(t.spender),
            ownerAccountId:
              null != t.owner ? s.default._fromProtobuf(t.owner) : null,
            amount: null != t.amount ? a.default.fromValue(t.amount) : null,
          });
        }
        static _fromGrantedProtobuf(t, e) {
          return new TokenAllowance({
            tokenId: r.default._fromProtobuf(t.tokenId),
            spenderAccountId: s.default._fromProtobuf(t.spender),
            ownerAccountId: e,
            amount: null != t.amount ? a.default.fromValue(t.amount) : null,
          });
        }
        _toProtobuf() {
          return {
            tokenId: this.tokenId._toProtobuf(),
            spender:
              null != this.spenderAccountId
                ? this.spenderAccountId._toProtobuf()
                : null,
            owner:
              null != this.ownerAccountId
                ? this.ownerAccountId._toProtobuf()
                : null,
            amount: this.amount,
          };
        }
        _validateChecksums(t) {
          (this.tokenId.validateChecksum(t),
            null != this.ownerAccountId &&
              this.ownerAccountId.validateChecksum(t),
            null != this.spenderAccountId &&
              this.spenderAccountId.validateChecksum(t));
        }
      }
      e.default = TokenAllowance;
    },
    5018: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = i(n(2978)),
        s = o(n(2774)),
        a = o(n(3028)),
        u = o(n(6120));
      function i(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (i = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountBalanceQuery extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            (this._contractId = null),
            null != t.accountId && this.setAccountId(t.accountId),
            null != t.contractId && this.setContractId(t.contractId));
        }
        static _fromProtobuf(t) {
          const e = t.cryptogetAccountBalance;
          return new AccountBalanceQuery({
            accountId:
              null != e.accountID
                ? s.default._fromProtobuf(e.accountID)
                : void 0,
            contractId:
              null != e.contractID
                ? a.default._fromProtobuf(e.contractID)
                : void 0,
          });
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        get contractId() {
          return this._contractId;
        }
        setContractId(t) {
          return (
            (this._contractId =
              "string" === typeof t ? a.default.fromString(t) : t.clone()),
            this
          );
        }
        _isPaymentRequired() {
          return !1;
        }
        _validateChecksums(t) {
          (null != this._accountId && this._accountId.validateChecksum(t),
            null != this._contractId && this._contractId.validateChecksum(t));
        }
        _execute(t, e) {
          return t.crypto.cryptoGetBalance(e);
        }
        _mapResponseHeader(t) {
          const e = t.cryptogetAccountBalance;
          return e.header;
        }
        _mapResponse(t, e, n) {
          const o = t.cryptogetAccountBalance;
          return Promise.resolve(u.default._fromProtobuf(o));
        }
        _onMakeRequest(t) {
          return {
            cryptogetAccountBalance: {
              header: t,
              accountID:
                null != this._accountId ? this._accountId._toProtobuf() : null,
              contractID:
                null != this._contractId
                  ? this._contractId._toProtobuf()
                  : null,
            },
          };
        }
        _getLogId() {
          return "AccountBalanceQuery:" + this._timestamp.toString();
        }
      }
      ((e.default = AccountBalanceQuery),
        r.QUERY_REGISTRY.set(
          "cryptogetAccountBalance",
          AccountBalanceQuery._fromProtobuf,
        ));
    },
    5019: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(3146)),
        a = o(n(3554));
      class LiveHash {
        constructor(t) {
          ((this.accountId = t.accountId),
            (this.hash = t.hash),
            (this.keys = t.keys),
            (this.duration = t.duration),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          const e = t;
          return new LiveHash({
            accountId: r.default._fromProtobuf(e.accountId),
            hash: null != e.hash ? e.hash : new Uint8Array(),
            keys:
              null != e.keys
                ? a.default.__fromProtobufKeyList(e.keys)
                : new a.default(),
            duration: s.default._fromProtobuf(e.duration),
          });
        }
        _toProtobuf() {
          return {
            accountId: this.accountId._toProtobuf(),
            hash: this.hash,
            keys: this.keys._toProtobufKey().keyList,
            duration: this.duration._toProtobuf(),
          };
        }
      }
      e.default = LiveHash;
    },
    5020: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = u(n(2978)),
        s = o(n(2774)),
        a = o(n(6121));
      o(n(2831));
      function u(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (u = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountInfoQuery extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            null != t.accountId && this.setAccountId(t.accountId));
        }
        static _fromProtobuf(t) {
          const e = t.cryptoGetInfo;
          return new AccountInfoQuery({
            accountId:
              null != e.accountID
                ? s.default._fromProtobuf(e.accountID)
                : void 0,
          });
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        _validateChecksums(t) {
          null != this._accountId && this._accountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.getAccountInfo(e);
        }
        async getCost(t) {
          return super.getCost(t);
        }
        _mapResponseHeader(t) {
          const e = t.cryptoGetInfo;
          return e.header;
        }
        _mapResponse(t, e, n) {
          const o = t.cryptoGetInfo;
          return Promise.resolve(a.default._fromProtobuf(o.accountInfo));
        }
        _onMakeRequest(t) {
          return {
            cryptoGetInfo: {
              header: t,
              accountID:
                null != this._accountId ? this._accountId._toProtobuf() : null,
            },
          };
        }
        _getLogId() {
          const t =
            null != this._paymentTransactionId &&
            null != this._paymentTransactionId.validStart
              ? this._paymentTransactionId.validStart
              : this._timestamp;
          return "AccountInfoQuery:" + t.toString();
        }
      }
      ((e.default = AccountInfoQuery),
        r.QUERY_REGISTRY.set("cryptoGetInfo", AccountInfoQuery._fromProtobuf));
    },
    6114: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(2774)),
        a = o(n(6115)),
        u = o(n(3295));
      class TokenTransferMap extends u.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
        __set(t, e, n) {
          const o = t.toString();
          let r = this._map.get(o);
          (null == r &&
            ((r = new a.default()), this._map.set(o, r), this.__map.set(t, r)),
            r._set(e, n));
        }
        static _fromProtobuf(t) {
          const e = new TokenTransferMap();
          for (const n of t) {
            const t = r.default._fromProtobuf(n.token);
            for (const o of null != n.transfers ? n.transfers : []) {
              const n = s.default._fromProtobuf(o.accountID);
              e.__set(t, n, o.amount);
            }
          }
          return e;
        }
        _toProtobuf() {
          const t = [];
          for (const [e, n] of this) {
            const o = [];
            for (const [t, e] of n)
              o.push({ accountID: t._toProtobuf(), amount: e });
            t.push({ token: e._toProtobuf(), transfers: o });
          }
          return t;
        }
      }
      e.default = TokenTransferMap;
    },
    6115: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(3295));
      class TokenTransferAccountMap extends s.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
        toJSON() {
          const t = {};
          return (
            this._map.forEach((e, n) => {
              t[n] = e.toString();
            }),
            t
          );
        }
      }
      e.default = TokenTransferAccountMap;
    },
    6116: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2754)),
        s = o(n(2835)),
        a = o(n(2774)),
        u = o(n(3295));
      class TokenNftTransferMap extends u.default {
        constructor() {
          super((t) => s.default.fromString(t));
        }
        __set(t, e) {
          const n = t.toString();
          let o = this._map.get(n);
          (null == o && ((o = []), this._map.set(n, o), this.__map.set(t, o)),
            o.push(e));
        }
        static _fromProtobuf(t) {
          const e = new TokenNftTransferMap();
          for (const n of t) {
            const t = s.default._fromProtobuf(n.token);
            for (const o of null != n.nftTransfers ? n.nftTransfers : []) {
              const n = a.default._fromProtobuf(o.senderAccountID),
                s = a.default._fromProtobuf(o.receiverAccountID);
              e.__set(t, {
                sender: n,
                recipient: s,
                serial: r.default.fromValue(o.serialNumber),
                isApproved: !1,
              });
            }
          }
          return e;
        }
        _toProtobuf() {
          const t = [];
          for (const [e, n] of this) {
            const o = [];
            for (const t of n)
              o.push({
                senderAccountID: t.sender._toProtobuf(),
                receiverAccountID: t.recipient._toProtobuf(),
                serialNumber: t.serial,
              });
            t.push({ token: e._toProtobuf(), nftTransfers: o });
          }
          return t;
        }
        toJSON() {
          const t = {};
          return (
            this._map.forEach((e, n) => {
              t[n] = e.map((t) => ({
                sender: t.sender.toString(),
                recipient: t.recipient.toString(),
                serial: t.serial,
                isApproved: t.isApproved,
              }));
            }),
            t
          );
        }
      }
      e.default = TokenNftTransferMap;
    },
    6120: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2754)),
        s = o(n(2831)),
        a = o(n(2835)),
        u = o(n(7916)),
        i = o(n(7917)),
        c = l(n(2826));
      function l(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (l = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountBalance {
        constructor(t) {
          ((this.hbars = t.hbars),
            (this.tokens = t.tokens),
            (this.tokenDecimals = t.tokenDecimals),
            Object.freeze(this));
        }
        static fromBytes(t) {
          return AccountBalance._fromProtobuf(
            c.proto.CryptoGetAccountBalanceResponse.decode(t),
          );
        }
        static _fromProtobuf(t) {
          const e = new u.default(),
            n = new i.default();
          if (null != t.tokenBalances)
            for (const o of t.tokenBalances) {
              const t = a.default._fromProtobuf(o.tokenId);
              (n._set(t, null != o.decimals ? o.decimals : 0),
                e._set(t, r.default.fromValue(o.balance)));
            }
          return new AccountBalance({
            hbars: s.default.fromTinybars(t.balance),
            tokens: e,
            tokenDecimals: n,
          });
        }
        _toProtobuf() {
          const t = [];
          for (const [e, n] of null != this.tokens ? this.tokens : [])
            t.push({
              tokenId: e._toProtobuf(),
              balance: n,
              decimals:
                null != this.tokenDecimals ? this.tokenDecimals.get(e) : null,
            });
          return { balance: this.hbars.toTinybars(), tokenBalances: t };
        }
        toBytes() {
          return c.proto.CryptoGetAccountBalanceResponse.encode(
            this._toProtobuf(),
          ).finish();
        }
        toString() {
          return JSON.stringify(this.toJSON());
        }
        toJSON() {
          const t = [];
          for (const [e, n] of null != this.tokens ? this.tokens : []) {
            const o =
              null != this.tokenDecimals ? this.tokenDecimals.get(e) : null;
            t.push({
              tokenId: e.toString(),
              balance: n.toString(),
              decimals: null != o ? o : 0,
            });
          }
          return { hbars: this.hbars.toString(), tokens: t };
        }
      }
      e.default = AccountBalance;
    },
    6121: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(6122)),
        a = o(n(5019)),
        u = o(n(2831)),
        i = o(n(2909)),
        c = o(n(2754)),
        l = o(n(6123)),
        d = m(n(2826)),
        f = o(n(3146)),
        h = o(n(2934)),
        _ = o(n(3421)),
        p = o(n(3375));
      function m(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (m = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountInfo {
        constructor(t) {
          ((this.accountId = t.accountId),
            (this.contractAccountId = t.contractAccountId),
            (this.isDeleted = t.isDeleted),
            (this.proxyAccountId = t.proxyAccountId),
            (this.proxyReceived = t.proxyReceived),
            (this.key = t.key),
            (this.balance = t.balance),
            (this.sendRecordThreshold = t.sendRecordThreshold),
            (this.receiveRecordThreshold = t.receiveRecordThreshold),
            (this.isReceiverSignatureRequired = t.isReceiverSignatureRequired),
            (this.expirationTime = t.expirationTime),
            (this.autoRenewPeriod = t.autoRenewPeriod),
            (this.liveHashes = t.liveHashes),
            (this.tokenRelationships = t.tokenRelationships),
            (this.accountMemo = t.accountMemo),
            (this.ownedNfts = t.ownedNfts),
            (this.maxAutomaticTokenAssociations =
              t.maxAutomaticTokenAssociations),
            (this.aliasKey = t.aliasKey),
            (this.ledgerId = t.ledgerId),
            (this.hbarAllowances = t.hbarAllowances),
            (this.tokenAllowances = t.tokenAllowances),
            (this.nftAllowances = t.nftAllowances),
            (this.ethereumNonce = t.ethereumNonce),
            (this.stakingInfo = t.stakingInfo),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          let e =
            null != t.alias && t.alias.length > 0
              ? h.default._fromProtobufKey(d.proto.Key.decode(t.alias))
              : null;
          e instanceof _.default || (e = null);
          const n = r.default._fromProtobuf(t.accountID);
          return new AccountInfo({
            accountId: n,
            contractAccountId:
              null != t.contractAccountID ? t.contractAccountID : null,
            isDeleted: null != t.deleted && t.deleted,
            key: h.default._fromProtobufKey(t.key),
            balance: u.default.fromTinybars(null != t.balance ? t.balance : 0),
            sendRecordThreshold: u.default.fromTinybars(
              null != t.generateSendRecordThreshold
                ? t.generateSendRecordThreshold
                : 0,
            ),
            receiveRecordThreshold: u.default.fromTinybars(
              null != t.generateReceiveRecordThreshold
                ? t.generateReceiveRecordThreshold
                : 0,
            ),
            isReceiverSignatureRequired:
              null != t.receiverSigRequired && t.receiverSigRequired,
            expirationTime: i.default._fromProtobuf(t.expirationTime),
            autoRenewPeriod:
              null != t.autoRenewPeriod
                ? new f.default(t.autoRenewPeriod.seconds)
                : new f.default(0),
            proxyAccountId:
              null != t.proxyAccountID &&
              0 !== c.default.fromValue(t.proxyAccountID.shardNum).toInt() &&
              0 !== c.default.fromValue(t.proxyAccountID.realmNum).toInt() &&
              0 !== c.default.fromValue(t.proxyAccountID.accountNum).toInt()
                ? r.default._fromProtobuf(t.proxyAccountID)
                : null,
            proxyReceived: u.default.fromTinybars(
              null != t.proxyReceived ? t.proxyReceived : 0,
            ),
            liveHashes: (null != t.liveHashes ? t.liveHashes : []).map((t) =>
              a.default._fromProtobuf(t),
            ),
            tokenRelationships: l.default._fromProtobuf(
              null != t.tokenRelationships ? t.tokenRelationships : [],
            ),
            accountMemo: null != t.memo ? t.memo : "",
            ownedNfts: t.ownedNfts ? t.ownedNfts : c.default.ZERO,
            maxAutomaticTokenAssociations: t.maxAutomaticTokenAssociations
              ? c.default.fromNumber(t.maxAutomaticTokenAssociations)
              : c.default.ZERO,
            aliasKey: e,
            ledgerId:
              null != t.ledgerId ? p.default.fromBytes(t.ledgerId) : null,
            hbarAllowances: [],
            tokenAllowances: [],
            nftAllowances: [],
            ethereumNonce: null != t.ethereumNonce ? t.ethereumNonce : null,
            stakingInfo:
              null != t.stakingInfo
                ? s.default._fromProtobuf(t.stakingInfo)
                : null,
          });
        }
        _toProtobuf() {
          return {
            accountID: this.accountId._toProtobuf(),
            contractAccountID: this.contractAccountId,
            deleted: this.isDeleted,
            proxyAccountID:
              null != this.proxyAccountId
                ? this.proxyAccountId._toProtobuf()
                : null,
            proxyReceived: this.proxyReceived.toTinybars(),
            key: this.key._toProtobufKey(),
            balance: this.balance.toTinybars(),
            generateSendRecordThreshold: this.sendRecordThreshold.toTinybars(),
            generateReceiveRecordThreshold:
              this.receiveRecordThreshold.toTinybars(),
            receiverSigRequired: this.isReceiverSignatureRequired,
            expirationTime: this.expirationTime._toProtobuf(),
            autoRenewPeriod: this.autoRenewPeriod._toProtobuf(),
            liveHashes: this.liveHashes.map((t) => t._toProtobuf()),
            tokenRelationships:
              null != this.tokenRelationships
                ? this.tokenRelationships._toProtobuf()
                : null,
            memo: this.accountMemo,
            ownedNfts: this.ownedNfts,
            maxAutomaticTokenAssociations:
              this.maxAutomaticTokenAssociations.toInt(),
            alias:
              null != this.aliasKey
                ? d.proto.Key.encode(this.aliasKey._toProtobufKey()).finish()
                : null,
            ledgerId: null != this.ledgerId ? this.ledgerId.toBytes() : null,
            ethereumNonce: this.ethereumNonce,
            stakingInfo:
              null != this.stakingInfo ? this.stakingInfo._toProtobuf() : null,
          };
        }
        static fromBytes(t) {
          return AccountInfo._fromProtobuf(
            d.proto.CryptoGetInfoResponse.AccountInfo.decode(t),
          );
        }
        toBytes() {
          return d.proto.CryptoGetInfoResponse.AccountInfo.encode(
            this._toProtobuf(),
          ).finish();
        }
        toString() {
          return JSON.stringify(this.toJSON());
        }
        toJSON() {
          return {
            balance: this.balance.toString(),
            accountId: this.accountId.toString(),
            contractAccountId: this.contractAccountId,
            isDeleted: this.isDeleted,
            proxyAccountId:
              null != this.proxyAccountId
                ? this.proxyAccountId.toString()
                : null,
            proxyReceived: this.proxyReceived.toString(),
            key: null != this.key ? this.key.toString() : null,
            sendRecordThreshold: this.sendRecordThreshold.toString(),
            receiveRecordThreshold: this.receiveRecordThreshold.toString(),
            isReceiverSignatureRequired: this.isReceiverSignatureRequired,
            expirationTime: this.expirationTime.toString(),
            autoRenewPeriod: this.autoRenewPeriod.toString(),
            accountMemo: this.accountMemo,
            ownedNfts: this.ownedNfts.toString(),
            maxAutomaticTokenAssociations:
              this.maxAutomaticTokenAssociations.toString(),
            aliasKey: null != this.aliasKey ? this.aliasKey.toString() : null,
            ledgerId: null != this.ledgerId ? this.ledgerId.toString() : null,
            ethereumNonce:
              null != this.ethereumNonce ? this.ethereumNonce.toString() : null,
            stakingInfo:
              null != this.stakingInfo ? this.stakingInfo.toJSON() : null,
          };
        }
      }
      e.default = AccountInfo;
    },
    6123: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(7920)),
        a = o(n(3295));
      class TokenRelationshipMap extends a.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
        static _fromProtobuf(t) {
          const e = new TokenRelationshipMap();
          for (const n of t) {
            const t = r.default._fromProtobuf(n.tokenId);
            e._set(t, s.default._fromProtobuf(n));
          }
          return e;
        }
        _toProtobuf() {
          const t = [];
          for (const [e, n] of this) t.push(n._toProtobuf());
          return t;
        }
      }
      e.default = TokenRelationshipMap;
    },
    6124: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = u(n(2978)),
        s = o(n(2774)),
        a = o(n(5013));
      function u(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (u = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountRecordsQuery extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            null != t.accountId && this.setAccountId(t.accountId));
        }
        static _fromProtobuf(t) {
          const e = t.cryptoGetAccountRecords;
          return new AccountRecordsQuery({
            accountId:
              null != e.accountID
                ? s.default._fromProtobuf(e.accountID)
                : void 0,
          });
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        _validateChecksums(t) {
          null != this._accountId && this._accountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.getAccountRecords(e);
        }
        _mapResponseHeader(t) {
          const e = t.cryptoGetAccountRecords;
          return e.header;
        }
        _mapResponse(t, e, n) {
          const o = t.cryptoGetAccountRecords,
            r = o.records;
          return Promise.resolve(
            r.map((t) => a.default._fromProtobuf({ transactionRecord: t })),
          );
        }
        _onMakeRequest(t) {
          return {
            cryptoGetAccountRecords: {
              header: t,
              accountID:
                null != this._accountId ? this._accountId._toProtobuf() : null,
            },
          };
        }
        _getLogId() {
          const t =
            null != this._paymentTransactionId &&
            null != this._paymentTransactionId.validStart
              ? this._paymentTransactionId.validStart
              : this._timestamp;
          return "AccountRecordsQuery:" + t.toString();
        }
      }
      ((e.default = AccountRecordsQuery),
        r.QUERY_REGISTRY.set(
          "cryptoGetAccountRecords",
          AccountRecordsQuery._fromProtobuf,
        ));
    },
    6125: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(2831));
      class ProxyStaker {
        constructor(t) {
          ((this.accountId = t.accountId),
            (this.amount =
              t.amount instanceof s.default
                ? t.amount
                : new s.default(t.amount)),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          return new ProxyStaker({
            accountId: r.default._fromProtobuf(t.accountID),
            amount: s.default.fromTinybars(null != t.amount ? t.amount : 0),
          });
        }
        _toProtobuf() {
          return {
            accountID: this.accountId._toProtobuf(),
            amount: this.amount.toTinybars(),
          };
        }
      }
      e.default = ProxyStaker;
    },
    6126: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(7925)),
        a = u(n(3374));
      function u(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (u = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class NodeAddress {
        constructor(t) {
          (void 0 === t && (t = {}),
            (this._publicKey = null),
            null != t.publicKey && this.setPublicKey(t.publicKey),
            (this._nodeId = null),
            null != t.nodeId && this.setNodeId(t.nodeId),
            (this._accountId = null),
            null != t.accountId && this.setAccountId(t.accountId),
            (this._certHash = null),
            null != t.certHash && this.setCertHash(t.certHash),
            (this._addresses = []),
            null != t.addresses && this.setAddresses(t.addresses),
            (this._description = null),
            null != t.description && this.setDescription(t.description),
            (this._stake = null),
            null != t.stake && this.setStake(t.stake));
        }
        get publicKey() {
          return this._publicKey;
        }
        setPublicKey(t) {
          return ((this._publicKey = t), this);
        }
        get nodeId() {
          return this._nodeId;
        }
        setNodeId(t) {
          return ((this._nodeId = t), this);
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            (this._accountId =
              "string" === typeof t ? r.default.fromString(t) : t.clone()),
            this
          );
        }
        get certHash() {
          return this._certHash;
        }
        setCertHash(t) {
          return ((this._certHash = t), this);
        }
        get addresses() {
          return this._addresses;
        }
        setAddresses(t) {
          return ((this._addresses = t), this);
        }
        get description() {
          return this._description;
        }
        setDescription(t) {
          return ((this._description = t), this);
        }
        get stake() {
          return this._stake;
        }
        setStake(t) {
          return ((this._stake = t), this);
        }
        static _fromProtobuf(t) {
          return new NodeAddress({
            publicKey: null != t.RSA_PubKey ? t.RSA_PubKey : void 0,
            nodeId: null != t.nodeId ? t.nodeId : void 0,
            accountId:
              null != t.nodeAccountId
                ? r.default._fromProtobuf(t.nodeAccountId)
                : void 0,
            certHash: null != t.nodeCertHash ? t.nodeCertHash : void 0,
            addresses:
              null != t.serviceEndpoint
                ? t.serviceEndpoint.map((t) => s.default._fromProtobuf(t))
                : void 0,
            description: null != t.description ? t.description : void 0,
            stake: null != t.stake ? t.stake : void 0,
          });
        }
        _toProtobuf() {
          return {
            RSA_PubKey: this._publicKey,
            nodeId: this._nodeId,
            nodeAccountId:
              null != this._accountId ? this._accountId._toProtobuf() : null,
            nodeCertHash: this._certHash,
            serviceEndpoint: this._addresses.map((t) => t._toProtobuf()),
            description: this._description,
            stake: this._stake,
          };
        }
        toString() {
          return JSON.stringify(this.toJSON());
        }
        toJSON() {
          return {
            publicKey: this._publicKey,
            nodeId: null != this._nodeId ? this._nodeId.toString() : null,
            accountId:
              null != this._accountId ? this._accountId.toString() : null,
            certHash: null != this._certHash ? a.decode(this._certHash) : null,
            addresses: this._addresses.map((t) => t.toJSON()),
            description: this._description,
            stake: null != this._stake ? this._stake.toString() : null,
          };
        }
      }
      e.default = NodeAddress;
    },
    6127: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(6126)),
        s = a(n(2826));
      function a(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (a = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class NodeAddressBook {
        constructor(t) {
          (void 0 === t && (t = {}),
            (this._nodeAddresses = []),
            null != t.nodeAddresses && this.setNodeAddresses(t.nodeAddresses));
        }
        get nodeAddresses() {
          return this._nodeAddresses;
        }
        setNodeAddresses(t) {
          return ((this._nodeAddresses = t), this);
        }
        static fromBytes(t) {
          return NodeAddressBook._fromProtobuf(
            s.proto.NodeAddressBook.decode(t),
          );
        }
        static _fromProtobuf(t) {
          return new NodeAddressBook({
            nodeAddresses:
              null != t.nodeAddress
                ? t.nodeAddress.map((t) => r.default._fromProtobuf(t))
                : void 0,
          });
        }
        _toProtobuf() {
          return {
            nodeAddress: this._nodeAddresses.map((t) => t._toProtobuf()),
          };
        }
        toString() {
          return JSON.stringify(this.toJSON());
        }
        toJSON() {
          return { nodeAddresses: this._nodeAddresses.map((t) => t.toJSON()) };
        }
        toBytes() {
          return s.proto.NodeAddressBook.encode(this._toProtobuf()).finish();
        }
      }
      e.default = NodeAddressBook;
    },
    7871: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2814)),
        s = o(n(2774)),
        a = o(n(3028)),
        u = o(n(2835)),
        i = o(n(3665)),
        c = o(n(2754)),
        l = o(n(2831)),
        d = o(n(5016)),
        f = o(n(5017)),
        h = o(n(4678)),
        _ = p(n(3215));
      function p(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (p = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountAllowanceAdjustTransaction extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._hbarAllowances =
              null != t.hbarAllowances ? t.hbarAllowances : []),
            (this._tokenAllowances =
              null != t.tokenAllowances ? t.tokenAllowances : []),
            (this._nftAllowances =
              null != t.nftAllowances ? t.nftAllowances : []));
        }
        get hbarAllowances() {
          return this._hbarAllowances;
        }
        addHbarAllowance(t, e) {
          const n = e instanceof l.default ? e : new l.default(e);
          return this._adjustHbarAllowance(null, t, _.requireNotNegative(n));
        }
        _adjustHbarAllowance(t, e, n) {
          return (
            this._requireNotFrozen(),
            this._hbarAllowances.push(
              new d.default({
                spenderAccountId:
                  "string" === typeof e
                    ? s.default.fromString(e)
                    : e instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                      : e,
                ownerAccountId:
                  "string" === typeof t
                    ? s.default.fromString(t)
                    : t instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, t.toSolidityAddress())
                      : t,
                amount: n,
              }),
            ),
            this
          );
        }
        grantHbarAllowance(t, e, n) {
          const o = n instanceof l.default ? n : new l.default(n);
          return this._adjustHbarAllowance(t, e, _.requireNotNegative(o));
        }
        revokeHbarAllowance(t, e, n) {
          const o = n instanceof l.default ? n : new l.default(n);
          return this._adjustHbarAllowance(
            t,
            e,
            _.requireNotNegative(o).negated(),
          );
        }
        get tokenAllowances() {
          return this._tokenAllowances;
        }
        addTokenAllowance(t, e, n) {
          return this._adjustTokenAllowance(
            t,
            null,
            e,
            _.requireNotNegative(c.default.fromValue(n)),
          );
        }
        _adjustTokenAllowance(t, e, n, o) {
          return (
            this._requireNotFrozen(),
            this._tokenAllowances.push(
              new f.default({
                tokenId: "string" === typeof t ? u.default.fromString(t) : t,
                spenderAccountId:
                  "string" === typeof n
                    ? s.default.fromString(n)
                    : n instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, n.toSolidityAddress())
                      : n,
                ownerAccountId:
                  "string" === typeof e
                    ? s.default.fromString(e)
                    : e instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                      : e,
                amount: "number" === typeof o ? c.default.fromNumber(o) : o,
              }),
            ),
            this
          );
        }
        grantTokenAllowance(t, e, n, o) {
          return this._adjustTokenAllowance(
            t,
            e,
            n,
            _.requireNotNegative(c.default.fromValue(o)),
          );
        }
        revokeTokenAllowance(t, e, n, o) {
          return this._adjustTokenAllowance(
            t,
            e,
            n,
            _.requireNotNegative(c.default.fromValue(o)),
          );
        }
        addTokenNftAllowance(t, e) {
          const n = "string" === typeof t ? i.default.fromString(t) : t;
          return this._adjustTokenNftAllowance(n, null, e);
        }
        _adjustTokenNftAllowance(t, e, n) {
          this._requireNotFrozen();
          const o =
              "string" === typeof n
                ? s.default.fromString(n)
                : n instanceof a.default
                  ? s.default.fromEvmAddress(0, 0, n.toSolidityAddress())
                  : n,
            r =
              "string" === typeof e
                ? s.default.fromString(e)
                : e instanceof a.default
                  ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                  : e;
          let u = !1;
          for (const s of this._nftAllowances)
            if (
              0 === s.tokenId.compare(t.tokenId) &&
              null != s.spenderAccountId &&
              0 === s.spenderAccountId.compare(o)
            ) {
              (null != s.serialNumbers && s.serialNumbers.push(t.serial),
                (u = !0));
              break;
            }
          return (
            u ||
              this._nftAllowances.push(
                new h.default({
                  tokenId: t.tokenId,
                  spenderAccountId: o,
                  serialNumbers: [t.serial],
                  ownerAccountId: r,
                  allSerials: !1,
                  delegatingSpender: null,
                }),
              ),
            this
          );
        }
        grantTokenNftAllowance(t, e, n) {
          const o = "string" === typeof t ? i.default.fromString(t) : t;
          return (
            _.requireNotNegative(o.serial),
            this._adjustTokenNftAllowance(o, e, n)
          );
        }
        revokeTokenNftAllowance(t, e, n) {
          const o = "string" === typeof t ? i.default.fromString(t) : t;
          return (
            _.requireNotNegative(o.serial),
            this._adjustTokenNftAllowance(
              new i.default(o.tokenId, o.serial.negate()),
              e,
              n,
            )
          );
        }
        addAllTokenNftAllowance(t, e) {
          return this._adjustTokenNftAllowanceAllSerials(t, null, e, !0);
        }
        grantTokenNftAllowanceAllSerials(t, e, n) {
          return this._adjustTokenNftAllowanceAllSerials(t, e, n, !0);
        }
        revokeTokenNftAllowanceAllSerials(t, e, n) {
          return this._adjustTokenNftAllowanceAllSerials(t, e, n, !1);
        }
        _adjustTokenNftAllowanceAllSerials(t, e, n, o) {
          return (
            this._requireNotFrozen(),
            this._nftAllowances.push(
              new h.default({
                tokenId: "string" === typeof t ? u.default.fromString(t) : t,
                ownerAccountId:
                  null != e
                    ? "string" === typeof e
                      ? s.default.fromString(e)
                      : e instanceof a.default
                        ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                        : e
                    : null,
                spenderAccountId:
                  "string" === typeof n
                    ? s.default.fromString(n)
                    : n instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, n.toSolidityAddress())
                      : n,
                serialNumbers: null,
                allSerials: o,
                delegatingSpender: null,
              }),
            ),
            this
          );
        }
        _validateChecksums(t) {
          (this._hbarAllowances.map((e) => e._validateChecksums(t)),
            this._tokenAllowances.map((e) => e._validateChecksums(t)),
            this._nftAllowances.map((e) => e._validateChecksums(t)));
        }
        _execute(t, e) {
          return Promise.reject(
            new Error("This feature has been deprecated for this class."),
          );
        }
        _getTransactionDataCase() {
          throw new Error("This feature has been deprecated for this class.");
        }
        _makeTransactionData() {
          throw new Error("This feature has been deprecated.");
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "AccountAllowanceAdjustTransaction:" + t.toString();
        }
      }
      e.default = AccountAllowanceAdjustTransaction;
    },
    7914: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = _(n(2814)),
        s = o(n(2774)),
        a = o(n(3028)),
        u = o(n(2835)),
        i = o(n(3665)),
        c = o(n(2754)),
        l = o(n(2831)),
        d = o(n(5016)),
        f = o(n(5017)),
        h = o(n(4678));
      function _(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (_ = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountAllowanceApproveTransaction extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._hbarApprovals =
              null != t.hbarApprovals ? t.hbarApprovals : []),
            (this._tokenApprovals =
              null != t.tokenApprovals ? t.tokenApprovals : []),
            (this._nftApprovals =
              null != t.nftApprovals ? t.nftApprovals : []));
        }
        static _fromProtobuf(t, e, n, o, s) {
          const a = s[0],
            u = a.cryptoApproveAllowance;
          return r.default._fromProtobufTransactions(
            new AccountAllowanceApproveTransaction({
              hbarApprovals: (null != u.cryptoAllowances
                ? u.cryptoAllowances
                : []
              ).map((t) => d.default._fromProtobuf(t)),
              tokenApprovals: (null != u.tokenAllowances
                ? u.tokenAllowances
                : []
              ).map((t) => f.default._fromProtobuf(t)),
              nftApprovals: (null != u.nftAllowances
                ? u.nftAllowances
                : []
              ).map((t) => h.default._fromProtobuf(t)),
            }),
            t,
            e,
            n,
            o,
            s,
          );
        }
        get hbarApprovals() {
          return this._hbarApprovals;
        }
        approveHbarAllowance(t, e, n) {
          return (
            this._requireNotFrozen(),
            this._hbarApprovals.push(
              new d.default({
                spenderAccountId:
                  "string" === typeof e
                    ? s.default.fromString(e)
                    : e instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                      : e,
                ownerAccountId:
                  "string" === typeof t
                    ? s.default.fromString(t)
                    : t instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, t.toSolidityAddress())
                      : t,
                amount: n instanceof l.default ? n : new l.default(n),
              }),
            ),
            this
          );
        }
        addHbarAllowance(t, e) {
          return (
            this._requireNotFrozen(),
            this._hbarApprovals.push(
              new d.default({
                spenderAccountId:
                  "string" === typeof t ? s.default.fromString(t) : t,
                amount: e instanceof l.default ? e : new l.default(e),
                ownerAccountId: null,
              }),
            ),
            this
          );
        }
        get tokenApprovals() {
          return this._tokenApprovals;
        }
        approveTokenAllowance(t, e, n, o) {
          return (
            this._requireNotFrozen(),
            this._tokenApprovals.push(
              new f.default({
                tokenId: "string" === typeof t ? u.default.fromString(t) : t,
                spenderAccountId:
                  "string" === typeof n
                    ? s.default.fromString(n)
                    : n instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, n.toSolidityAddress())
                      : n,
                ownerAccountId:
                  "string" === typeof e
                    ? s.default.fromString(e)
                    : e instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                      : e,
                amount: "number" === typeof o ? c.default.fromNumber(o) : o,
              }),
            ),
            this
          );
        }
        addTokenAllowance(t, e, n) {
          return (
            this._requireNotFrozen(),
            this._tokenApprovals.push(
              new f.default({
                tokenId: "string" === typeof t ? u.default.fromString(t) : t,
                spenderAccountId:
                  "string" === typeof e ? s.default.fromString(e) : e,
                amount: "number" === typeof n ? c.default.fromNumber(n) : n,
                ownerAccountId: null,
              }),
            ),
            this
          );
        }
        addTokenNftAllowance(t, e) {
          return this._approveTokenNftAllowance(t, null, e, null);
        }
        get tokenNftApprovals() {
          return this._nftApprovals;
        }
        _approveTokenNftAllowance(t, e, n, o) {
          this._requireNotFrozen();
          const r = "string" === typeof t ? i.default.fromString(t) : t,
            u =
              "string" === typeof n
                ? s.default.fromString(n)
                : n instanceof a.default
                  ? s.default.fromEvmAddress(0, 0, n.toSolidityAddress())
                  : n;
          let c = !1;
          for (const s of this._nftApprovals)
            if (
              0 === s.tokenId.compare(r.tokenId) &&
              null != s.spenderAccountId &&
              0 === s.spenderAccountId.compare(u)
            ) {
              (null != s.serialNumbers && s.serialNumbers.push(r.serial),
                (c = !0));
              break;
            }
          return (
            c ||
              this._nftApprovals.push(
                new h.default({
                  tokenId: r.tokenId,
                  spenderAccountId: u,
                  ownerAccountId:
                    "string" === typeof e
                      ? s.default.fromString(e)
                      : e instanceof a.default
                        ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                        : e,
                  serialNumbers: [r.serial],
                  allSerials: !1,
                  delegatingSpender:
                    "string" === typeof o ? s.default.fromString(o) : o,
                }),
              ),
            this
          );
        }
        approveTokenNftAllowance(t, e, n) {
          return this._approveTokenNftAllowance(t, e, n, null);
        }
        approveTokenNftAllowanceWithDelegatingSpender(t, e, n, o) {
          return this._approveTokenNftAllowance(t, e, n, o);
        }
        _approveAllTokenNftAllowance(t, e, n, o) {
          return (
            this._requireNotFrozen(),
            this._nftApprovals.push(
              new h.default({
                tokenId: "string" === typeof t ? u.default.fromString(t) : t,
                spenderAccountId:
                  "string" === typeof n
                    ? s.default.fromString(n)
                    : n instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, n.toSolidityAddress())
                      : n,
                ownerAccountId:
                  "string" === typeof e
                    ? s.default.fromString(e)
                    : e instanceof a.default
                      ? s.default.fromEvmAddress(0, 0, e.toSolidityAddress())
                      : e,
                serialNumbers: null,
                allSerials: o,
                delegatingSpender: null,
              }),
            ),
            this
          );
        }
        addAllTokenNftAllowance(t, e, n) {
          return this._approveAllTokenNftAllowance(t, e, n, !0);
        }
        approveTokenNftAllowanceAllSerials(t, e, n) {
          return this._approveAllTokenNftAllowance(t, e, n, !0);
        }
        deleteTokenNftAllowanceAllSerials(t, e, n) {
          return this._approveAllTokenNftAllowance(t, e, n, !1);
        }
        _validateChecksums(t) {
          (this._hbarApprovals.map((e) => e._validateChecksums(t)),
            this._tokenApprovals.map((e) => e._validateChecksums(t)),
            this._nftApprovals.map((e) => e._validateChecksums(t)));
        }
        _execute(t, e) {
          return t.crypto.approveAllowances(e);
        }
        _getTransactionDataCase() {
          return "cryptoApproveAllowance";
        }
        _makeTransactionData() {
          return {
            cryptoAllowances: this._hbarApprovals.map((t) => t._toProtobuf()),
            tokenAllowances: this._tokenApprovals.map((t) => t._toProtobuf()),
            nftAllowances: this._nftApprovals.map((t) => t._toProtobuf()),
          };
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "AccountAllowanceApproveTransaction:" + t.toString();
        }
      }
      ((e.default = AccountAllowanceApproveTransaction),
        r.TRANSACTION_REGISTRY.set(
          "cryptoApproveAllowance",
          AccountAllowanceApproveTransaction._fromProtobuf,
        ));
    },
    7915: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = i(n(2814)),
        s = o(n(2774)),
        a = o(n(3665)),
        u = o(n(4678));
      function i(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (i = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountAllowanceDeleteTransaction extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._nftAllowances =
              null != t.nftAllowances ? t.nftAllowances : []));
        }
        static _fromProtobuf(t, e, n, o, s) {
          const a = s[0],
            i = a.cryptoDeleteAllowance;
          return r.default._fromProtobufTransactions(
            new AccountAllowanceDeleteTransaction({
              nftAllowances: (null != i.nftAllowances
                ? i.nftAllowances
                : []
              ).map((t) => u.default._fromProtobuf(t)),
            }),
            t,
            e,
            n,
            o,
            s,
          );
        }
        get tokenNftAllowanceDeletions() {
          return this._nftAllowances;
        }
        deleteAllTokenNftAllowances(t, e) {
          this._requireNotFrozen();
          const n = "string" === typeof t ? a.default.fromString(t) : t,
            o = "string" === typeof e ? s.default.fromString(e) : e;
          let r = !1;
          for (const s of this._nftAllowances)
            if (0 === s.tokenId.compare(n.tokenId)) {
              (null != s.serialNumbers && s.serialNumbers.push(n.serial),
                (r = !0));
              break;
            }
          return (
            r ||
              this._nftAllowances.push(
                new u.default({
                  tokenId: n.tokenId,
                  spenderAccountId: null,
                  serialNumbers: [n.serial],
                  ownerAccountId: o,
                  allSerials: !1,
                  delegatingSpender: null,
                }),
              ),
            this
          );
        }
        _validateChecksums(t) {
          this._nftAllowances.map((e) => e._validateChecksums(t));
        }
        _execute(t, e) {
          return t.crypto.deleteAllowances(e);
        }
        _getTransactionDataCase() {
          return "cryptoDeleteAllowance";
        }
        _makeTransactionData() {
          return {
            nftAllowances: this._nftAllowances.map((t) => t._toProtobuf()),
          };
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "AccountAllowanceDeleteTransaction:" + t.toString();
        }
      }
      ((e.default = AccountAllowanceDeleteTransaction),
        r.TRANSACTION_REGISTRY.set(
          "cryptoDeleteAllowance",
          AccountAllowanceDeleteTransaction._fromProtobuf,
        ));
    },
    7916: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(3295));
      class TokenBalanceMap extends s.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
      }
      e.default = TokenBalanceMap;
    },
    7917: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(3295));
      class TokenDecimalMap extends s.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
      }
      e.default = TokenDecimalMap;
    },
    7918: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      d(n(2826));
      var r = o(n(2831)),
        s = o(n(2774)),
        a = d(n(2814)),
        u = o(n(3146)),
        i = o(n(2754)),
        c = o(n(2934)),
        l = o(n(4292));
      function d(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (d = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountCreateTransaction extends a.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._key = null),
            (this._initialBalance = null),
            (this._sendRecordThreshold = a.DEFAULT_RECORD_THRESHOLD),
            (this._receiveRecordThreshold = a.DEFAULT_RECORD_THRESHOLD),
            (this._receiverSignatureRequired = !1),
            (this._proxyAccountId = null),
            (this._autoRenewPeriod = new u.default(
              a.DEFAULT_AUTO_RENEW_PERIOD,
            )),
            (this._accountMemo = null),
            (this._maxAutomaticTokenAssociations = null),
            (this._stakedAccountId = null),
            (this._stakedNodeId = null),
            (this._declineStakingReward = !1),
            (this._alias = null),
            null != t.key && this.setKey(t.key),
            null != t.receiverSignatureRequired &&
              this.setReceiverSignatureRequired(t.receiverSignatureRequired),
            null != t.initialBalance &&
              this.setInitialBalance(t.initialBalance),
            null != t.proxyAccountId &&
              this.setProxyAccountId(t.proxyAccountId),
            null != t.autoRenewPeriod &&
              this.setAutoRenewPeriod(t.autoRenewPeriod),
            null != t.accountMemo && this.setAccountMemo(t.accountMemo),
            null != t.maxAutomaticTokenAssociations &&
              this.setMaxAutomaticTokenAssociations(
                t.maxAutomaticTokenAssociations,
              ),
            null != t.stakedAccountId &&
              this.setStakedAccountId(t.stakedAccountId),
            null != t.stakedNodeId && this.setStakedNodeId(t.stakedNodeId),
            null != t.declineStakingReward &&
              this.setDeclineStakingReward(t.declineStakingReward),
            null != t.alias && this.setAlias(t.alias));
        }
        static _fromProtobuf(t, e, n, o, r) {
          const u = r[0],
            i = u.cryptoCreateAccount;
          let d = void 0;
          return (
            null != i.alias &&
              i.alias.length > 0 &&
              20 === i.alias.length &&
              (d = l.default.fromBytes(i.alias)),
            a.default._fromProtobufTransactions(
              new AccountCreateTransaction({
                key: null != i.key ? c.default._fromProtobufKey(i.key) : void 0,
                initialBalance:
                  null != i.initialBalance ? i.initialBalance : void 0,
                receiverSignatureRequired:
                  null != i.receiverSigRequired
                    ? i.receiverSigRequired
                    : void 0,
                proxyAccountId:
                  null != i.proxyAccountID
                    ? s.default._fromProtobuf(i.proxyAccountID)
                    : void 0,
                autoRenewPeriod:
                  null != i.autoRenewPeriod && null != i.autoRenewPeriod.seconds
                    ? i.autoRenewPeriod.seconds
                    : void 0,
                accountMemo: null != i.memo ? i.memo : void 0,
                maxAutomaticTokenAssociations:
                  null != i.maxAutomaticTokenAssociations
                    ? i.maxAutomaticTokenAssociations
                    : void 0,
                stakedAccountId:
                  null != i.stakedAccountId
                    ? s.default._fromProtobuf(i.stakedAccountId)
                    : void 0,
                stakedNodeId: null != i.stakedNodeId ? i.stakedNodeId : void 0,
                declineStakingReward: 1 == i.declineReward,
                alias: d,
              }),
              t,
              e,
              n,
              o,
              r,
            )
          );
        }
        get key() {
          return this._key;
        }
        setKey(t) {
          return (this._requireNotFrozen(), (this._key = t), this);
        }
        get initialBalance() {
          return this._initialBalance;
        }
        setInitialBalance(t) {
          return (
            this._requireNotFrozen(),
            (this._initialBalance =
              t instanceof r.default ? t : new r.default(t)),
            this
          );
        }
        get receiverSignatureRequired() {
          return this._receiverSignatureRequired;
        }
        setReceiverSignatureRequired(t) {
          return (
            this._requireNotFrozen(),
            (this._receiverSignatureRequired = t),
            this
          );
        }
        get proxyAccountId() {
          return this._proxyAccountId;
        }
        setProxyAccountId(t) {
          return (this._requireNotFrozen(), (this._proxyAccountId = t), this);
        }
        get autoRenewPeriod() {
          return this._autoRenewPeriod;
        }
        setAutoRenewPeriod(t) {
          return (
            this._requireNotFrozen(),
            (this._autoRenewPeriod =
              t instanceof u.default ? t : new u.default(t)),
            this
          );
        }
        get accountMemo() {
          return this._accountMemo;
        }
        setAccountMemo(t) {
          return (this._requireNotFrozen(), (this._accountMemo = t), this);
        }
        get maxAutomaticTokenAssociations() {
          return this._maxAutomaticTokenAssociations;
        }
        setMaxAutomaticTokenAssociations(t) {
          return (
            this._requireNotFrozen(),
            (this._maxAutomaticTokenAssociations =
              "number" === typeof t ? i.default.fromNumber(t) : t),
            this
          );
        }
        get stakedAccountId() {
          return this._stakedAccountId;
        }
        setStakedAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._stakedAccountId =
              "string" === typeof t ? s.default.fromString(t) : t),
            this
          );
        }
        get stakedNodeId() {
          return this._stakedNodeId;
        }
        setStakedNodeId(t) {
          return (
            this._requireNotFrozen(),
            (this._stakedNodeId = i.default.fromValue(t)),
            this
          );
        }
        get declineStakingRewards() {
          return this._declineStakingReward;
        }
        setDeclineStakingReward(t) {
          return (
            this._requireNotFrozen(),
            (this._declineStakingReward = t),
            this
          );
        }
        get alias() {
          return this._alias;
        }
        setAlias(t) {
          if ("string" === typeof t) {
            if (!((t.startsWith("0x") && 42 == t.length) || 40 == t.length))
              throw new Error(
                'evmAddress must be a valid EVM address with or without "0x" prefix',
              );
            this._alias = l.default.fromString(t);
          } else this._alias = t;
          return this;
        }
        _validateChecksums(t) {
          null != this._proxyAccountId &&
            this._proxyAccountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.createAccount(e);
        }
        _getTransactionDataCase() {
          return "cryptoCreateAccount";
        }
        _makeTransactionData() {
          let t = null;
          return (
            null != this._alias && (t = this._alias.toBytes()),
            {
              key: null != this._key ? this._key._toProtobufKey() : null,
              initialBalance:
                null != this._initialBalance
                  ? this._initialBalance.toTinybars()
                  : null,
              autoRenewPeriod: this._autoRenewPeriod._toProtobuf(),
              proxyAccountID:
                null != this._proxyAccountId
                  ? this._proxyAccountId._toProtobuf()
                  : null,
              receiveRecordThreshold: this._receiveRecordThreshold.toTinybars(),
              sendRecordThreshold: this._sendRecordThreshold.toTinybars(),
              receiverSigRequired: this._receiverSignatureRequired,
              memo: this._accountMemo,
              maxAutomaticTokenAssociations:
                null != this._maxAutomaticTokenAssociations
                  ? this._maxAutomaticTokenAssociations.toInt()
                  : null,
              stakedAccountId:
                null != this.stakedAccountId
                  ? this.stakedAccountId._toProtobuf()
                  : null,
              stakedNodeId: this.stakedNodeId,
              declineReward: this.declineStakingRewards,
              alias: t,
            }
          );
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "AccountCreateTransaction:" + t.toString();
        }
      }
      ((e.default = AccountCreateTransaction),
        a.TRANSACTION_REGISTRY.set(
          "cryptoCreateAccount",
          AccountCreateTransaction._fromProtobuf,
        ));
    },
    7919: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = a(n(2814));
      function a(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (a = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountDeleteTransaction extends s.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            (this._transferAccountId = null),
            null != t.accountId && this.setAccountId(t.accountId),
            null != t.transferAccountId &&
              this.setTransferAccountId(t.transferAccountId));
        }
        static _fromProtobuf(t, e, n, o, a) {
          const u = a[0],
            i = u.cryptoDelete;
          return s.default._fromProtobufTransactions(
            new AccountDeleteTransaction({
              accountId:
                null != i.deleteAccountID
                  ? r.default._fromProtobuf(i.deleteAccountID)
                  : void 0,
              transferAccountId:
                null != i.transferAccountID
                  ? r.default._fromProtobuf(i.transferAccountID)
                  : void 0,
            }),
            t,
            e,
            n,
            o,
            a,
          );
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._accountId =
              "string" === typeof t ? r.default.fromString(t) : t.clone()),
            this
          );
        }
        get transferAccountId() {
          return this._transferAccountId;
        }
        setTransferAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._transferAccountId =
              "string" === typeof t ? r.default.fromString(t) : t.clone()),
            this
          );
        }
        _validateChecksums(t) {
          (null != this._accountId && this._accountId.validateChecksum(t),
            null != this._transferAccountId &&
              this._transferAccountId.validateChecksum(t));
        }
        _execute(t, e) {
          return t.crypto.cryptoDelete(e);
        }
        _getTransactionDataCase() {
          return "cryptoDelete";
        }
        _makeTransactionData() {
          return {
            deleteAccountID:
              null != this._accountId ? this._accountId._toProtobuf() : null,
            transferAccountID:
              null != this._transferAccountId
                ? this._transferAccountId._toProtobuf()
                : null,
          };
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "AccountDeleteTransaction:" + t.toString();
        }
      }
      ((e.default = AccountDeleteTransaction),
        s.TRANSACTION_REGISTRY.set(
          "cryptoDelete",
          AccountDeleteTransaction._fromProtobuf,
        ));
    },
    7920: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2754)),
        s = o(n(2835));
      class TokenRelationship {
        constructor(t) {
          ((this.tokenId = t.tokenId),
            (this.symbol = t.symbol),
            (this.balance = t.balance),
            (this.isKycGranted = t.isKycGranted),
            (this.isFrozen = t.isFrozen),
            (this.automaticAssociation = t.automaticAssociation),
            Object.freeze(this));
        }
        static _fromProtobuf(t) {
          const e = s.default._fromProtobuf(t.tokenId),
            n =
              null == t.kycStatus || 0 === t.kycStatus
                ? null
                : 1 === t.kycStatus,
            o =
              null == t.freezeStatus || 0 === t.freezeStatus
                ? null
                : 1 === t.freezeStatus;
          return new TokenRelationship({
            tokenId: e,
            symbol: t.symbol,
            balance:
              null != t.balance
                ? t.balance instanceof r.default
                  ? t.balance
                  : r.default.fromValue(t.balance)
                : r.default.ZERO,
            isKycGranted: n,
            isFrozen: o,
            automaticAssociation:
              null != t.automaticAssociation ? t.automaticAssociation : null,
          });
        }
        _toProtobuf() {
          return {
            tokenId: this.tokenId._toProtobuf(),
            symbol: this.symbol,
            balance: this.balance,
            kycStatus:
              null == this.isKycGranted ? 0 : this.isKycGranted ? 1 : 2,
            freezeStatus: null == this.isFrozen ? 0 : this.isFrozen ? 1 : 2,
            automaticAssociation: this.automaticAssociation,
          };
        }
      }
      e.default = TokenRelationship;
    },
    7921: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(5020)),
        s = o(n(3554));
      class AccountInfoFlow {
        static async verifySignature(t, e, n, o) {
          const a = await new r.default().setAccountId(e).execute(t);
          return !(a.key instanceof s.default) && a.key.verify(n, o);
        }
        static async verifyTransaction(t, e, n) {
          const o = await new r.default().setAccountId(e).execute(t);
          return !(o.key instanceof s.default) && o.key.verifyTransaction(n);
        }
        static async verifySignatureWithSigner(t, e, n, o) {
          const a = await new r.default().setAccountId(e).executeWithSigner(t);
          return !(a.key instanceof s.default) && a.key.verify(n, o);
        }
        static async verifyTransactionWithSigner(t, e, n) {
          const o = await new r.default().setAccountId(e).executeWithSigner(t);
          return !(o.key instanceof s.default) && o.key.verifyTransaction(n);
        }
      }
      e.default = AccountInfoFlow;
    },
    7922: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = u(n(2978)),
        s = o(n(2774)),
        a = o(n(6125));
      function u(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (u = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountStakersQuery extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            null != t.accountId && this.setAccountId(t.accountId));
        }
        static _fromProtobuf(t) {
          const e = t.cryptoGetProxyStakers;
          return new AccountStakersQuery({
            accountId:
              null != e.accountID
                ? s.default._fromProtobuf(e.accountID)
                : void 0,
          });
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        _validateChecksums(t) {
          null != this._accountId && this._accountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.getStakersByAccountID(e);
        }
        _mapResponseHeader(t) {
          const e = t.cryptoGetProxyStakers;
          return e.header;
        }
        _mapResponse(t) {
          const e = t.cryptoGetProxyStakers,
            n = e.stakers;
          return Promise.resolve(
            (null != n.proxyStaker ? n.proxyStaker : []).map((t) =>
              a.default._fromProtobuf(t),
            ),
          );
        }
        _onMakeRequest(t) {
          return {
            cryptoGetProxyStakers: {
              header: t,
              accountID:
                null != this._accountId ? this._accountId._toProtobuf() : null,
            },
          };
        }
        _getLogId() {
          const t =
            null != this._paymentTransactionId &&
            null != this._paymentTransactionId.validStart
              ? this._paymentTransactionId.validStart
              : this._timestamp;
          return "AccountStakersQuery:" + t.toString();
        }
      }
      ((e.default = AccountStakersQuery),
        r.QUERY_REGISTRY.set(
          "cryptoGetProxyStakers",
          AccountStakersQuery._fromProtobuf,
        ));
    },
    7923: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = l(n(2814)),
        s = o(n(2774)),
        a = o(n(2909)),
        u = o(n(3146)),
        i = o(n(2754)),
        c = o(n(2934));
      function l(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (l = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class AccountUpdateTransaction extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            (this._key = null),
            (this._receiverSignatureRequired = !1),
            (this._proxyAccountId = null),
            (this._autoRenewPeriod = null),
            (this._expirationTime = null),
            (this._accountMemo = null),
            (this._maxAutomaticTokenAssociations = null),
            (this._aliasKey = null),
            (this._stakedAccountId = null),
            (this._stakedNodeId = null),
            (this._declineStakingReward = null),
            null != t.accountId && this.setAccountId(t.accountId),
            null != t.key && this.setKey(t.key),
            null != t.receiverSignatureRequired &&
              this.setReceiverSignatureRequired(t.receiverSignatureRequired),
            null != t.proxyAccountId &&
              this.setProxyAccountId(t.proxyAccountId),
            null != t.autoRenewPeriod &&
              this.setAutoRenewPeriod(t.autoRenewPeriod),
            null != t.expirationTime &&
              this.setExpirationTime(t.expirationTime),
            null != t.accountMemo && this.setAccountMemo(t.accountMemo),
            null != t.maxAutomaticTokenAssociations &&
              this.setMaxAutomaticTokenAssociations(
                t.maxAutomaticTokenAssociations,
              ),
            null != t.stakedAccountId &&
              this.setStakedAccountId(t.stakedAccountId),
            null != t.stakedNodeId && this.setStakedNodeId(t.stakedNodeId),
            null != t.declineStakingReward &&
              this.setDeclineStakingReward(t.declineStakingReward));
        }
        static _fromProtobuf(t, e, n, o, u) {
          const l = u[0],
            d = l.cryptoUpdateAccount;
          return r.default._fromProtobufTransactions(
            new AccountUpdateTransaction({
              accountId:
                null != d.accountIDToUpdate
                  ? s.default._fromProtobuf(d.accountIDToUpdate)
                  : void 0,
              key: null != d.key ? c.default._fromProtobufKey(d.key) : void 0,
              receiverSignatureRequired:
                null != d.receiverSigRequired ? d.receiverSigRequired : void 0,
              proxyAccountId:
                null != d.proxyAccountID
                  ? s.default._fromProtobuf(d.proxyAccountID)
                  : void 0,
              autoRenewPeriod:
                null != d.autoRenewPeriod && null != d.autoRenewPeriod.seconds
                  ? d.autoRenewPeriod.seconds
                  : void 0,
              expirationTime:
                null != d.expirationTime
                  ? a.default._fromProtobuf(d.expirationTime)
                  : void 0,
              accountMemo:
                null != d.memo && null != d.memo.value ? d.memo.value : void 0,
              maxAutomaticTokenAssociations:
                null != d.maxAutomaticTokenAssociations &&
                null != d.maxAutomaticTokenAssociations.value
                  ? i.default.fromNumber(d.maxAutomaticTokenAssociations.value)
                  : void 0,
              stakedAccountId:
                null != d.stakedAccountId
                  ? s.default._fromProtobuf(d.stakedAccountId)
                  : void 0,
              stakedNodeId: null != d.stakedNodeId ? d.stakedNodeId : void 0,
              declineStakingReward:
                null != d.declineReward && 1 == Boolean(d.declineReward),
            }),
            t,
            e,
            n,
            o,
            u,
          );
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        get key() {
          return this._key;
        }
        setKey(t) {
          return (this._requireNotFrozen(), (this._key = t), this);
        }
        get receiverSignatureRequired() {
          return this._receiverSignatureRequired;
        }
        setReceiverSignatureRequired(t) {
          return (
            this._requireNotFrozen(),
            (this._receiverSignatureRequired = t),
            this
          );
        }
        get proxyAccountId() {
          return this._proxyAccountId;
        }
        setProxyAccountId(t) {
          return (this._requireNotFrozen(), (this._proxyAccountId = t), this);
        }
        get autoRenewPeriod() {
          return this._autoRenewPeriod;
        }
        setAutoRenewPeriod(t) {
          return (
            this._requireNotFrozen(),
            (this._autoRenewPeriod =
              t instanceof u.default ? t : new u.default(t)),
            this
          );
        }
        get expirationTime() {
          return this._expirationTime;
        }
        setExpirationTime(t) {
          return (
            this._requireNotFrozen(),
            (this._expirationTime =
              t instanceof Date ? a.default.fromDate(t) : t),
            this
          );
        }
        get accountMemo() {
          return this._accountMemo;
        }
        setAccountMemo(t) {
          return (this._requireNotFrozen(), (this._accountMemo = t), this);
        }
        clearAccountMemo() {
          return (this._requireNotFrozen(), (this._accountMemo = null), this);
        }
        get maxAutomaticTokenAssociations() {
          return this._maxAutomaticTokenAssociations;
        }
        setMaxAutomaticTokenAssociations(t) {
          return (
            this._requireNotFrozen(),
            (this._maxAutomaticTokenAssociations =
              "number" === typeof t ? i.default.fromNumber(t) : t),
            this
          );
        }
        get aliasKey() {
          return null;
        }
        setAliasKey(t) {
          return this;
        }
        get stakedAccountId() {
          return this._stakedAccountId;
        }
        setStakedAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._stakedAccountId =
              "string" === typeof t ? s.default.fromString(t) : t),
            this
          );
        }
        clearStakedAccountId() {
          return (
            this._requireNotFrozen(),
            (this._stakedAccountId = new s.default(0, 0, 0)),
            this
          );
        }
        get stakedNodeId() {
          return this._stakedNodeId;
        }
        setStakedNodeId(t) {
          return (
            this._requireNotFrozen(),
            (this._stakedNodeId = i.default.fromValue(t)),
            this
          );
        }
        clearStakedNodeId() {
          return (
            this._requireNotFrozen(),
            (this._stakedNodeId = i.default.fromNumber(-1)),
            this
          );
        }
        get declineStakingRewards() {
          return this._declineStakingReward;
        }
        setDeclineStakingReward(t) {
          return (
            this._requireNotFrozen(),
            (this._declineStakingReward = t),
            this
          );
        }
        _validateChecksums(t) {
          (null != this._accountId && this._accountId.validateChecksum(t),
            null != this._proxyAccountId &&
              this._proxyAccountId.validateChecksum(t));
        }
        _execute(t, e) {
          return t.crypto.updateAccount(e);
        }
        _getTransactionDataCase() {
          return "cryptoUpdateAccount";
        }
        _makeTransactionData() {
          return {
            accountIDToUpdate:
              null != this._accountId ? this._accountId._toProtobuf() : null,
            key: null != this._key ? this._key._toProtobufKey() : null,
            expirationTime:
              null != this._expirationTime
                ? this._expirationTime._toProtobuf()
                : null,
            proxyAccountID:
              null != this._proxyAccountId
                ? this._proxyAccountId._toProtobuf()
                : null,
            autoRenewPeriod:
              null != this._autoRenewPeriod
                ? this._autoRenewPeriod._toProtobuf()
                : null,
            receiverSigRequiredWrapper:
              null == this._receiverSignatureRequired
                ? null
                : { value: this._receiverSignatureRequired },
            memo:
              null != this._accountMemo ? { value: this._accountMemo } : null,
            maxAutomaticTokenAssociations:
              null != this._maxAutomaticTokenAssociations
                ? { value: this._maxAutomaticTokenAssociations.toInt() }
                : null,
            stakedAccountId:
              null != this.stakedAccountId
                ? this.stakedAccountId._toProtobuf()
                : null,
            stakedNodeId: this.stakedNodeId,
            declineReward:
              null != this.declineStakingRewards
                ? { value: this.declineStakingRewards }
                : null,
          };
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "AccountUpdateTransaction:" + t.toString();
        }
      }
      ((e.default = AccountUpdateTransaction),
        r.TRANSACTION_REGISTRY.set(
          "cryptoUpdateAccount",
          AccountUpdateTransaction._fromProtobuf,
        ));
    },
    7925: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(7926));
      class EndPoint {
        constructor(t) {
          (void 0 === t && (t = {}),
            (this._address = null),
            null != t.address && this.setAddress(t.address),
            (this._port = null),
            null != t.port && this.setPort(t.port));
        }
        get address() {
          return this.address;
        }
        setAddress(t) {
          return ((this._address = t), this);
        }
        get port() {
          return this._port;
        }
        setPort(t) {
          return ((this._port = t), this);
        }
        static _fromProtobuf(t) {
          return new EndPoint({
            address:
              null != t.ipAddressV4
                ? r.default._fromProtobuf(t.ipAddressV4)
                : void 0,
            port: null != t.port ? t.port : void 0,
          });
        }
        _toProtobuf() {
          return {
            ipAddressV4:
              null != this._address ? this._address._toProtobuf() : null,
            port: this._port,
          };
        }
        toString() {
          return `${null != this._address ? this._address.toString() : ""}:${null != this._port ? this._port.toString() : ""}`;
        }
        toJSON() {
          return {
            address: null != this._address ? this._address.toString() : null,
            port: null != this._port ? this._port.toString() : null,
          };
        }
      }
      e.default = EndPoint;
    },
    7926: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(7927));
      class IPv4Address {
        constructor(t) {
          (void 0 === t && (t = {}),
            (this._network = null),
            null != t.network && this.setNetwork(t.network),
            (this._host = null),
            null != t.host && this.setHost(t.host));
        }
        get newtork() {
          return this._network;
        }
        setNetwork(t) {
          return ((this._network = t), this);
        }
        get host() {
          return this._host;
        }
        setHost(t) {
          return ((this._host = t), this);
        }
        static _fromProtobuf(t) {
          return new IPv4Address({
            network: new r.default().setLeft(t[0]).setRight(t[1]),
            host: new r.default().setLeft(t[2]).setRight(t[3]),
          });
        }
        _toProtobuf() {
          return Uint8Array.of(
            null != this._network && null != this._network._left
              ? this._network._left
              : 0,
            null != this._network && null != this._network.right
              ? this._network.right
              : 0,
            null != this._host && null != this._host.left ? this._host.left : 0,
            null != this._host && null != this._host.right
              ? this._host.right
              : 0,
          );
        }
        toString() {
          return null != this._network && null != this._host
            ? `${this._network.toString()}.${this._host.toString()}`
            : "";
        }
      }
      e.default = IPv4Address;
    },
    7927: function (t, e, n) {
      "use strict";
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      class IPv4AddressPart {
        constructor(t) {
          (void 0 === t && (t = {}),
            (this._left = null),
            null != t.left && this.setLeft(t.left),
            (this._right = null),
            null != t.right && this.setRight(t.right));
        }
        get left() {
          return this._left;
        }
        setLeft(t) {
          return ((this._left = t), this);
        }
        get right() {
          return this._right;
        }
        setRight(t) {
          return ((this._right = t), this);
        }
        toString() {
          return null != this._left && null != this._right
            ? `${this._left.toString()}.${this._right.toString()}`
            : "";
        }
      }
      e.default = IPv4AddressPart;
    },
    7948: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = c(n(2814)),
        s = o(n(2774)),
        a = o(n(3146)),
        u = o(n(2934)),
        i = o(n(3554));
      function c(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (c = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class LiveHashAddTransaction extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._hash = null),
            (this._keys = null),
            (this._duration = null),
            (this._accountId = null),
            null != t.hash && this.setHash(t.hash),
            null != t.keys && this.setKeys(t.keys),
            null != t.duration && this.setDuration(t.duration),
            null != t.accountId && this.setAccountId(t.accountId));
        }
        static _fromProtobuf(t, e, n, o, a) {
          const i = a[0],
            c = i.cryptoAddLiveHash,
            l = c.liveHash;
          return r.default._fromProtobufTransactions(
            new LiveHashAddTransaction({
              hash: null != l.hash ? l.hash : void 0,
              keys:
                null != l.keys && null != l.keys.keys
                  ? l.keys.keys.map((t) => u.default._fromProtobufKey(t))
                  : void 0,
              duration:
                null != l.duration && null != l.duration.seconds
                  ? l.duration.seconds
                  : void 0,
              accountId:
                null != l.accountId
                  ? s.default._fromProtobuf(l.accountId)
                  : void 0,
            }),
            t,
            e,
            n,
            o,
            a,
          );
        }
        get hash() {
          return this._hash;
        }
        setHash(t) {
          return (this._requireNotFrozen(), (this._hash = t), this);
        }
        get keys() {
          return this._keys;
        }
        setKeys(t) {
          return (
            this._requireNotFrozen(),
            (this._keys = t instanceof i.default ? t.toArray() : t),
            this
          );
        }
        get duration() {
          return this._duration;
        }
        setDuration(t) {
          return (
            this._requireNotFrozen(),
            (this._duration = t instanceof a.default ? t : new a.default(t)),
            this
          );
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        _validateChecksums(t) {
          null != this._accountId && this._accountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.addLiveHash(e);
        }
        _getTransactionDataCase() {
          return "cryptoAddLiveHash";
        }
        _makeTransactionData() {
          return {
            liveHash: {
              hash: this._hash,
              keys:
                null != this._keys
                  ? { keys: this._keys.map((t) => t._toProtobufKey()) }
                  : void 0,
              duration:
                null != this._duration ? this._duration._toProtobuf() : null,
              accountId:
                null != this._accountId ? this._accountId._toProtobuf() : null,
            },
          };
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "LiveHashAddTransaction:" + t.toString();
        }
      }
      ((e.default = LiveHashAddTransaction),
        r.TRANSACTION_REGISTRY.set(
          "cryptoAddLiveHash",
          LiveHashAddTransaction._fromProtobuf,
        ));
    },
    7949: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = a(n(2814)),
        s = o(n(2774));
      function a(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (a = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class LiveHashDeleteTransaction extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._hash = null),
            (this._accountId = null),
            null != t.hash && this.setHash(t.hash),
            null != t.accountId && this.setAccountId(t.accountId));
        }
        static _fromProtobuf(t, e, n, o, a) {
          const u = a[0],
            i = u.cryptoDeleteLiveHash;
          return r.default._fromProtobufTransactions(
            new LiveHashDeleteTransaction({
              hash: null != i.liveHashToDelete ? i.liveHashToDelete : void 0,
              accountId:
                null != i.accountOfLiveHash
                  ? s.default._fromProtobuf(i.accountOfLiveHash)
                  : void 0,
            }),
            t,
            e,
            n,
            o,
            a,
          );
        }
        get hash() {
          return this._hash;
        }
        setHash(t) {
          return (this._requireNotFrozen(), (this._hash = t), this);
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            this._requireNotFrozen(),
            (this._accountId =
              "string" === typeof t ? s.default.fromString(t) : t.clone()),
            this
          );
        }
        _validateChecksums(t) {
          null != this._accountId && this._accountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.deleteLiveHash(e);
        }
        _getTransactionDataCase() {
          return "cryptoDeleteLiveHash";
        }
        _makeTransactionData() {
          return {
            liveHashToDelete: this._hash,
            accountOfLiveHash:
              null != this._accountId ? this._accountId._toProtobuf() : null,
          };
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "LiveHashDeleteTransaction:" + t.toString();
        }
      }
      ((e.default = LiveHashDeleteTransaction),
        r.TRANSACTION_REGISTRY.set(
          "cryptoDeleteLiveHash",
          LiveHashDeleteTransaction._fromProtobuf,
        ));
    },
    7950: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = u(n(2978)),
        s = o(n(2774)),
        a = o(n(5019));
      function u(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (u = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class LiveHashQuery extends r.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._accountId = null),
            null != t.accountId && this.setAccountId(t.accountId),
            (this._hash = null),
            null != t.hash && this.setHash(t.hash));
        }
        static _fromProtobuf(t) {
          const e = t.cryptoGetLiveHash;
          return new LiveHashQuery({
            accountId:
              null != e.accountID
                ? s.default._fromProtobuf(e.accountID)
                : void 0,
            hash: null != e.hash ? e.hash : void 0,
          });
        }
        get accountId() {
          return this._accountId;
        }
        setAccountId(t) {
          return (
            (this._accountId =
              t instanceof s.default ? t : s.default.fromString(t)),
            this
          );
        }
        get liveHash() {
          return this._hash;
        }
        setHash(t) {
          return ((this._hash = t), this);
        }
        _validateChecksums(t) {
          null != this._accountId && this._accountId.validateChecksum(t);
        }
        _execute(t, e) {
          return t.crypto.getLiveHash(e);
        }
        _mapResponseHeader(t) {
          const e = t.cryptoGetLiveHash;
          return e.header;
        }
        _mapResponse(t) {
          const e = t.cryptoGetLiveHash;
          return Promise.resolve(a.default._fromProtobuf(e.liveHash));
        }
        _onMakeRequest(t) {
          return {
            cryptoGetLiveHash: {
              header: t,
              accountID:
                null != this._accountId ? this._accountId._toProtobuf() : null,
              hash: this._hash,
            },
          };
        }
        _getLogId() {
          const t =
            null != this._paymentTransactionId &&
            null != this._paymentTransactionId.validStart
              ? this._paymentTransactionId.validStart
              : this._timestamp;
          return "LiveHashQuery:" + t.toString();
        }
      }
      ((e.default = LiveHashQuery),
        r.QUERY_REGISTRY.set("cryptoGetLiveHash", LiveHashQuery._fromProtobuf));
    },
    7984: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2831)),
        s = o(n(2835)),
        a = o(n(2774)),
        u = I(n(2814)),
        i = o(n(2754)),
        c = o(n(7985)),
        l = o(n(5014)),
        d = o(n(6118)),
        f = o(n(6114)),
        h = o(n(7986)),
        _ = o(n(6116)),
        p = o(n(6115)),
        m = o(n(7987)),
        A = o(n(3665));
      function I(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (I = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class TransferTransaction extends u.default {
        constructor(t) {
          (void 0 === t && (t = {}),
            super(),
            (this._tokenTransfers = []),
            (this._hbarTransfers = []),
            (this._nftTransfers = []),
            (this._defaultMaxTransactionFee = new r.default(1)));
          for (const e of null != t.tokenTransfers ? t.tokenTransfers : [])
            this.addTokenTransfer(e.tokenId, e.accountId, e.amount);
          for (const e of null != t.hbarTransfers ? t.hbarTransfers : [])
            this.addHbarTransfer(e.accountId, e.amount);
          for (const e of null != t.nftTransfers ? t.nftTransfers : [])
            this.addNftTransfer(e.tokenId, e.serial, e.sender, e.recipient);
        }
        static _fromProtobuf(t, e, n, o, r) {
          const s = r[0],
            a = s.cryptoTransfer,
            i = new TransferTransaction();
          return (
            (i._tokenTransfers = d.default._fromProtobuf(
              null != a.tokenTransfers ? a.tokenTransfers : [],
            )),
            (i._hbarTransfers = l.default._fromProtobuf(
              null != a.transfers && null != a.transfers.accountAmounts
                ? a.transfers.accountAmounts
                : [],
            )),
            (i._nftTransfers = m.default._fromProtobuf(
              null != a.tokenTransfers ? a.tokenTransfers : [],
            )),
            u.default._fromProtobufTransactions(i, t, e, n, o, r)
          );
        }
        get tokenTransfers() {
          const t = new f.default();
          for (const e of this._tokenTransfers) {
            let n = t.get(e.tokenId);
            null != n
              ? n._set(e.accountId, e.amount)
              : ((n = new p.default()),
                n._set(e.accountId, e.amount),
                t._set(e.tokenId, n));
          }
          return t;
        }
        _addTokenTransfer(t, e, n, o) {
          this._requireNotFrozen();
          const r = t instanceof s.default ? t : s.default.fromString(t),
            u = e instanceof a.default ? e : a.default.fromString(e),
            c = n instanceof i.default ? n : i.default.fromNumber(n);
          for (const s of this._tokenTransfers)
            if (0 === s.tokenId.compare(r) && 0 === s.accountId.compare(u))
              return (
                (s.amount = s.amount.add(c)),
                (s.expectedDecimals = null),
                this
              );
          return (
            this._tokenTransfers.push(
              new d.default({
                tokenId: t,
                accountId: e,
                expectedDecimals: null,
                amount: n,
                isApproved: o,
              }),
            ),
            this
          );
        }
        addTokenTransfer(t, e, n) {
          return this._addTokenTransfer(t, e, n, !1);
        }
        addApprovedTokenTransfer(t, e, n) {
          return this._addTokenTransfer(t, e, n, !0);
        }
        addTokenTransferWithDecimals(t, e, n, o) {
          this._requireNotFrozen();
          const r = t instanceof s.default ? t : s.default.fromString(t),
            u = e instanceof a.default ? e : a.default.fromString(e),
            c = n instanceof i.default ? n : i.default.fromNumber(n);
          let l = !1;
          for (const s of this._tokenTransfers)
            if (0 === s.tokenId.compare(r)) {
              if (null != s.expectedDecimals && s.expectedDecimals !== o)
                throw new Error("expected decimals mis-match");
              ((s.expectedDecimals = o),
                0 === s.accountId.compare(u) &&
                  ((s.amount = s.amount.add(c)),
                  (s.expectedDecimals = o),
                  (l = !0)));
            }
          return (
            l ||
              this._tokenTransfers.push(
                new d.default({
                  tokenId: t,
                  accountId: e,
                  expectedDecimals: o,
                  amount: n,
                  isApproved: !1,
                }),
              ),
            this
          );
        }
        get tokenIdDecimals() {
          const t = new c.default();
          for (const e of this._tokenTransfers)
            t._set(e.tokenId, e.expectedDecimals);
          return t;
        }
        get hbarTransfers() {
          const t = new h.default();
          for (const e of this._hbarTransfers) t._set(e.accountId, e.amount);
          return t;
        }
        _addHbarTransfer(t, e, n) {
          this._requireNotFrozen();
          const o =
              t instanceof a.default ? t.clone() : a.default.fromString(t),
            s = e instanceof r.default ? e : new r.default(e);
          for (const a of this._hbarTransfers)
            if (0 === a.accountId.compare(o))
              return (
                (a.amount = r.default.fromTinybars(
                  a.amount.toTinybars().add(s.toTinybars()),
                )),
                this
              );
          return (
            this._hbarTransfers.push(
              new l.default({ accountId: o, amount: s, isApproved: n }),
            ),
            this
          );
        }
        addHbarTransfer(t, e) {
          return this._addHbarTransfer(t, e, !1);
        }
        addApprovedHbarTransfer(t, e) {
          return this._addHbarTransfer(t, e, !0);
        }
        _validateChecksums(t) {
          for (const e of this._hbarTransfers) e.accountId.validateChecksum(t);
          for (const e of this._tokenTransfers)
            (e.tokenId.validateChecksum(t), e.accountId.validateChecksum(t));
          for (const e of this._nftTransfers)
            (e.tokenId.validateChecksum(t),
              e.senderAccountId.validateChecksum(t),
              e.receiverAccountId.validateChecksum(t));
        }
        get nftTransfers() {
          const t = new _.default();
          for (const e of this._nftTransfers) {
            const n = t.get(e.tokenId),
              o = {
                sender: e.senderAccountId,
                recipient: e.receiverAccountId,
                serial: e.serialNumber,
                isApproved: e.isApproved,
              };
            null != n ? n.push(o) : t._set(e.tokenId, [o]);
          }
          return t;
        }
        _addNftTransfer(t, e, n, o, r) {
          let u, i, c;
          if ((this._requireNotFrozen(), e instanceof A.default))
            ((u = e),
              (i = "string" === typeof n ? a.default.fromString(n) : n),
              (c = "string" === typeof o ? a.default.fromString(o) : o));
          else if (e instanceof s.default)
            ((u = new A.default(e, n)),
              (i = "string" === typeof o ? a.default.fromString(o) : o),
              (c = "string" === typeof r ? a.default.fromString(r) : r));
          else
            try {
              ((u = A.default.fromString(e)),
                (i = "string" === typeof n ? a.default.fromString(n) : n),
                (c = "string" === typeof o ? a.default.fromString(o) : o));
            } catch (l) {
              const t = s.default.fromString(e);
              ((u = new A.default(t, n)),
                (i = "string" === typeof o ? a.default.fromString(o) : o),
                (c = "string" === typeof r ? a.default.fromString(r) : r));
            }
          for (const s of this._nftTransfers)
            if (
              0 === s.tokenId.compare(u.tokenId) &&
              0 === s.serialNumber.compare(u.serial)
            )
              return ((s.senderAccountId = i), (s.receiverAccountId = c), this);
          return (
            this._nftTransfers.push(
              new m.default({
                tokenId: u.tokenId,
                serialNumber: u.serial,
                senderAccountId: i,
                receiverAccountId: c,
                isApproved: t,
              }),
            ),
            this
          );
        }
        addNftTransfer(t, e, n, o) {
          return this._addNftTransfer(!1, t, e, n, o);
        }
        addApprovedNftTransfer(t, e, n, o) {
          return this._addNftTransfer(!0, t, e, n, o);
        }
        setHbarTransferApproval(t, e) {
          const n = "string" === typeof t ? a.default.fromString(t) : t;
          for (const o of this._hbarTransfers)
            0 === o.accountId.compare(n) && (o.isApproved = e);
          return this;
        }
        setTokenTransferApproval(t, e, n) {
          const o = "string" === typeof t ? s.default.fromString(t) : t,
            r = "string" === typeof e ? a.default.fromString(e) : e;
          for (const s of this._tokenTransfers)
            0 === s.tokenId.compare(o) &&
              0 === s.accountId.compare(r) &&
              (s.isApproved = n);
          return this;
        }
        setNftTransferApproval(t, e) {
          const n = "string" === typeof t ? A.default.fromString(t) : t;
          for (const o of this._nftTransfers)
            0 === o.tokenId.compare(n.tokenId) &&
              0 === o.serialNumber.compare(n.serial) &&
              (o.isApproved = e);
          return this;
        }
        _execute(t, e) {
          return t.crypto.cryptoTransfer(e);
        }
        _getTransactionDataCase() {
          return "cryptoTransfer";
        }
        _makeTransactionData() {
          const t = [];
          (this._tokenTransfers.sort((t, e) => {
            const n = t.tokenId.compare(e.tokenId);
            return 0 !== n ? n : t.accountId.compare(e.accountId);
          }),
            this._nftTransfers.sort((t, e) => {
              const n = t.senderAccountId.compare(e.senderAccountId);
              if (0 != n) return n;
              const o = t.receiverAccountId.compare(e.receiverAccountId);
              return 0 != o ? o : t.serialNumber.compare(e.serialNumber);
            }));
          let e = 0,
            n = 0;
          while (
            e < this._tokenTransfers.length ||
            n < this._nftTransfers.length
          )
            if (
              e < this._tokenTransfers.length &&
              n < this._nftTransfers.length
            ) {
              const o = this._tokenTransfers[e].tokenId,
                r = this._nftTransfers[n].tokenId,
                s = t.length > 0 ? t[t.length - 1] : null,
                a = null != s ? s.tokenId : null;
              if (null != s && null != a && 0 === a.compare(o)) {
                s.transfers.push(this._tokenTransfers[e++]);
                continue;
              }
              if (null != s && null != a && 0 === a.compare(r)) {
                s.nftTransfers.push(this._nftTransfers[n++]);
                continue;
              }
              const u = o.compare(r);
              0 === u
                ? t.push({
                    tokenId: o,
                    expectedDecimals: this._tokenTransfers[e].expectedDecimals,
                    transfers: [this._tokenTransfers[e++]],
                    nftTransfers: [this._nftTransfers[n++]],
                  })
                : u < 0
                  ? t.push({
                      tokenId: o,
                      expectedDecimals:
                        this._tokenTransfers[e].expectedDecimals,
                      transfers: [this._tokenTransfers[e++]],
                      nftTransfers: [],
                    })
                  : t.push({
                      tokenId: r,
                      expectedDecimals: null,
                      transfers: [],
                      nftTransfers: [this._nftTransfers[n++]],
                    });
            } else if (e < this._tokenTransfers.length) {
              const n = this._tokenTransfers[e].tokenId;
              let o;
              for (const e of t) 0 === e.tokenId.compare(n) && (o = e);
              const r = null != o ? o.tokenId : null;
              if (null != o && null != r && 0 === r.compare(n)) {
                o.transfers.push(this._tokenTransfers[e++]);
                continue;
              }
              t.push({
                tokenId: n,
                expectedDecimals: this._tokenTransfers[e].expectedDecimals,
                transfers: [this._tokenTransfers[e++]],
                nftTransfers: [],
              });
            } else if (n < this._nftTransfers.length) {
              const e = this._nftTransfers[n].tokenId;
              let o;
              for (const n of t) 0 === n.tokenId.compare(e) && (o = n);
              const r = null != o ? o.tokenId : null;
              if (null != o && null != r && 0 === r.compare(e)) {
                o.nftTransfers.push(this._nftTransfers[n++]);
                continue;
              }
              t.push({
                tokenId: e,
                expectedDecimals: null,
                transfers: [],
                nftTransfers: [this._nftTransfers[n++]],
              });
            }
          return (
            this._hbarTransfers.sort((t, e) =>
              t.accountId.compare(e.accountId),
            ),
            {
              transfers: {
                accountAmounts: this._hbarTransfers.map((t) => ({
                  accountID: t.accountId._toProtobuf(),
                  amount: t.amount.toTinybars(),
                  isApproval: t.isApproved,
                })),
              },
              tokenTransfers: t.map((t) => ({
                token: t.tokenId._toProtobuf(),
                expectedDecimals:
                  null != t.expectedDecimals
                    ? { value: t.expectedDecimals }
                    : null,
                transfers: t.transfers.map((t) => t._toProtobuf()),
                nftTransfers: t.nftTransfers.map((t) => t._toProtobuf()),
              })),
            }
          );
        }
        _getLogId() {
          const t = this._transactionIds.current.validStart;
          return "TransferTransaction:" + t.toString();
        }
      }
      ((e.default = TransferTransaction),
        u.TRANSACTION_REGISTRY.set(
          "cryptoTransfer",
          TransferTransaction._fromProtobuf,
        ));
    },
    7985: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2835)),
        s = o(n(3295));
      class NullableTokenDecimalMap extends s.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
      }
      e.default = NullableTokenDecimalMap;
    },
    7986: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(2774)),
        s = o(n(2831)),
        a = o(n(3295));
      class HbarTransferMap extends a.default {
        constructor() {
          super((t) => r.default.fromString(t));
        }
        static _fromProtobuf(t) {
          const e = new HbarTransferMap();
          for (const n of null != t.accountAmounts ? t.accountAmounts : []) {
            const t = r.default._fromProtobuf(n.accountID);
            e._set(t, s.default.fromTinybars(n.amount));
          }
          return e;
        }
      }
      e.default = HbarTransferMap;
    },
    7988: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.default = void 0));
      var r = o(n(5007)),
        s = o(n(2774)),
        a = o(n(6144)),
        u = o(n(5018)),
        i = o(n(5020)),
        c = o(n(6124)),
        l = o(n(3093)),
        d = f(n(3215));
      function f(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (f = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      class Wallet {
        constructor(t, e, n) {
          const o = "string" === typeof e ? r.default.fromStringDer(e) : e;
          ((this.publicKey = o.publicKey),
            (this.signer = (t) => Promise.resolve(o.sign(t))),
            (this.provider = n),
            (this.accountId =
              "string" === typeof t ? s.default.fromString(t) : t));
        }
        static createRandomED25519() {
          const t = r.default.generateED25519(),
            e = t.publicKey,
            n = e.toAccountId(0, 0);
          return Promise.resolve(new Wallet(n, t));
        }
        static createRandomECDSA() {
          const t = r.default.generateECDSA(),
            e = t.publicKey,
            n = e.toAccountId(0, 0);
          return Promise.resolve(new Wallet(n, t));
        }
        getProvider() {
          return this.provider;
        }
        getAccountId() {
          return this.accountId;
        }
        getAccountKey() {
          return this.publicKey;
        }
        getLedgerId() {
          return null == this.provider ? null : this.provider.getLedgerId();
        }
        getNetwork() {
          return null == this.provider ? {} : this.provider.getNetwork();
        }
        getMirrorNetwork() {
          return null == this.provider ? [] : this.provider.getMirrorNetwork();
        }
        async sign(t) {
          const e = [];
          for (const n of t)
            e.push(
              new a.default({
                publicKey: this.publicKey,
                signature: await this.signer(n),
                accountId: this.accountId,
              }),
            );
          return e;
        }
        getAccountBalance() {
          return this.call(new u.default().setAccountId(this.accountId));
        }
        getAccountInfo() {
          return this.call(new i.default().setAccountId(this.accountId));
        }
        getAccountRecords() {
          return this.call(new c.default().setAccountId(this.accountId));
        }
        signTransaction(t) {
          return t.signWith(this.publicKey, this.signer);
        }
        checkTransaction(t) {
          const e = t.transactionId;
          if (
            null != e &&
            null != e.accountId &&
            0 != e.accountId.compare(this.accountId)
          )
            throw new Error(
              "transaction's ID constructed with a different account ID",
            );
          if (null == this.provider) return Promise.resolve(t);
          const n = (null != t.nodeAccountIds ? t.nodeAccountIds : []).map(
              (t) => t.toString(),
            ),
            o = Object.values(this.provider.getNetwork()).map((t) =>
              t.toString(),
            );
          if (!n.reduce((t, e) => t && o.includes(e), !0))
            throw new Error(
              "Transaction already set node account IDs to values not within the current network",
            );
          return Promise.resolve(t);
        }
        populateTransaction(t) {
          if (
            (t._freezeWithAccountId(this.accountId),
            null == t.transactionId &&
              t.setTransactionId(l.default.generate(this.accountId)),
            null != t.nodeAccountIds && 0 != t.nodeAccountIds.length)
          )
            return Promise.resolve(t.freeze());
          if (null == this.provider) return Promise.resolve(t);
          const e = Object.values(this.provider.getNetwork()).map((t) =>
            "string" === typeof t ? s.default.fromString(t) : t,
          );
          return (
            d.shuffle(e),
            t.setNodeAccountIds(e.slice(0, (e.length + 3 - 1) / 3)),
            Promise.resolve(t.freeze())
          );
        }
        call(t) {
          if (null == this.provider)
            throw new Error(
              "cannot send request with an wallet that doesn't contain a provider",
            );
          return this.provider.call(
            t._setOperatorWith(this.accountId, this.publicKey, this.signer),
          );
        }
      }
      e.default = Wallet;
    },
    7998: function (t, e, n) {
      "use strict";
      var o = n(9);
      (Object.defineProperty(e, "__esModule", { value: !0 }),
        (e.TESTNET_ADDRESS_BOOK =
          e.PREVIEWNET_ADDRESS_BOOK =
          e.MAINNET_ADDRESS_BOOK =
            void 0));
      var r = o(n(6127)),
        s = u(n(2950)),
        a = u(n(2826));
      function u(t, e) {
        if ("function" == typeof WeakMap)
          var n = new WeakMap(),
            o = new WeakMap();
        return (u = function (t, e) {
          if (!e && t && t.__esModule) return t;
          var r,
            s,
            a = { __proto__: null, default: t };
          if (null === t || ("object" != typeof t && "function" != typeof t))
            return a;
          if ((r = e ? o : n)) {
            if (r.has(t)) return r.get(t);
            r.set(t, a);
          }
          for (const n in t)
            "default" !== n &&
              {}.hasOwnProperty.call(t, n) &&
              ((s =
                (r = Object.defineProperty) &&
                Object.getOwnPropertyDescriptor(t, n)) &&
              (s.get || s.set)
                ? r(a, n, s)
                : (a[n] = t[n]));
          return a;
        })(t, e);
      }
      ((e.PREVIEWNET_ADDRESS_BOOK = r.default._fromProtobuf(
        a.proto.NodeAddressBook.decode(
          s.decode(
            "",
          ),
        ),
      )),
        (e.TESTNET_ADDRESS_BOOK = r.default._fromProtobuf(
          a.proto.NodeAddressBook.decode(
            s.decode(
              "",
            ),
          ),
        )),
        (e.MAINNET_ADDRESS_BOOK = r.default._fromProtobuf(
          a.proto.NodeAddressBook.decode(
            s.decode(
              "",
            ),
          ),
        )));
    },
  },
]);
